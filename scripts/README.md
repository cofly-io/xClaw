...
- Builds the console frontend (`console/`), copies `console/dist` to `src/xclaw/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `xclaw:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/xclaw:v1 --no-cache`.
