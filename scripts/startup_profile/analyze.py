...
# -*- coding: utf-8 -*-
"""
xClaw Startup Performance Analyzer
...
def parse_import_log(log_file):
    ...
    # Filter xClaw (package root) and third-party
    xclaw_import_list = [i for i in imports if "xclaw" in i["package"]]
    third_party = [
        i
        for i in imports
    ...
    xclaw_import_list.sort(key=lambda x: x["cumulative_ms"], reverse=True)
    third_party.sort(key=lambda x: x["cumulative_ms"], reverse=True)

    total_xclaw = sum(
        i["cumulative_ms"]
        for i in xclaw_import_list
        if i["package"].count(".") == 1
    )
    total_third_party = sum(i["cumulative_ms"] for i in third_party[:10])

    print(
        f"   ✓ {len(imports)} imports, {len(xclaw_import_list)} xClaw, "
        f"{len(third_party)} third-party",
    )

    return {
        "xclaw_imports": xclaw_import_list,
        "third_party_imports": third_party,
        "summary": {
            "total_ms": total_xclaw + total_third_party,
            "total_xclaw_ms": total_xclaw,
...
def parse_execution_trace(trace_file):
    ...
    try:
        ...
        xclaw_funcs = [f for f in functions if "xclaw" in f["function"]]

        print(
            f"   ✓ {len(functions)} functions, {len(xclaw_funcs)} xClaw",
        )

        return {
            "xclaw_functions": xclaw_funcs[:50],
...
def main():  # pylint: disable=too-many-statements
    """Main function."""
    print("=" * 60)
    print("🐾 xClaw Startup Performance Analyzer")
    print("=" * 60)
    print()
