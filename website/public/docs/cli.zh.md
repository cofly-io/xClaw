...
`xclaw` 是 xClaw 的命令行工具。本页按「上手 → 配置 → 日常管理」的顺序组织——
新用户从头读，老用户直接跳到需要的章节。
...
```bash
xclaw init              # 交互式初始化（推荐新用户）
xclaw init --defaults   # 不交互，用默认值（适合脚本）
xclaw init --force      # 覆盖已有配置文件
```
...
启动 xClaw 服务。频道、定时任务、控制台等所有运行时功能都依赖此服务。

```bash
xclaw app                             # 默认 127.0.0.1:8088
xclaw app --reload                    # 代码改动自动重载（开发用）
xclaw app --log-level debug           # 详细日志
```
...
| `--workers`   | —           | **[已废弃]** 将被忽略，xClaw 始终使用 1 个 worker             |

> **说明：** `--workers` 选项因稳定性原因已废弃。xClaw 被设计为单 worker 进程运行。多 worker 模式会导致内存状态管理和 WebSocket 连接出现问题。此选项将在未来版本中移除。

### 控制台

`xclaw app` 启动后，在浏览器打开 `http://127.0.0.1:8088/` 即可进入 **控制台** ——
一个用于对话、频道、定时任务、技能、模型等的 Web 管理界面。详见 [控制台](./console)。

若未构建前端，根路径会返回类似 `{"message": "xClaw Web Console is not available."}` 的提示信息（实际文案可能调整），API 仍可正常使用。
...
| 命令                         | 说明                                                                           |
| ---------------------------- | ------------------------------------------------------------------------------ |
| `xclaw daemon status`        | 状态（配置、工作目录、记忆服务）                                               |
| `xclaw daemon restart`       | 打印说明（在对话中用 /daemon restart 可进程内重载）                            |
| `xclaw daemon reload-config` | 重新读取并校验配置（频道/MCP 变更需在对话中 /daemon restart 或重启进程后生效） |
| `xclaw daemon version`       | 版本与路径                                                                     |
| `xclaw daemon logs [-n N]`   | 最近 N 行日志（默认 100，来自工作目录 `xclaw.log`）                            |

**多智能体支持：** 所有命令都支持 `--agent-id` 参数（默认为 `default`）。

```bash
xclaw daemon status                     # 默认智能体状态
xclaw daemon status --agent-id abc123   # 特定智能体状态
...
使用 xClaw 前至少需要配置一个 LLM 提供商。环境变量为内置工具（如网页搜索）提供凭据。
...
| 命令                                   | 说明                                   |
| -------------------------------------- | -------------------------------------- |
| `xclaw models list`                    | 查看所有提供商、API Key 状态和当前模型 |
| `xclaw models config`                  | 完整交互式配置：API Key → 选择模型     |
| `xclaw models config-key [provider]`   | 单独配置某个提供商的 API Key           |
| `xclaw models set-llm`                 | 只切换活跃模型（不改 API Key）         |
| `xclaw models local`                   | 查看已下载的本地模型                   |
| `xclaw models download <repo_id>`      | 下载一个本地模型（llama.cpp）          |
| `xclaw models remove-local <model_id>` | 删除已下载的本地模型                   |

```bash
xclaw models list                    # 看当前状态
xclaw models config                  # 完整交互式配置
xclaw models config-key modelscope   # 只配 ModelScope 的 API Key
xclaw models config-key dashscope    # 只配 DashScope 的 API Key
xclaw models config-key custom       # 配置自定义提供商（Base URL + Key）
xclaw models set-llm                 # 只切换模型
```

#### 本地模型

xClaw 也支持通过 llama.cpp，Ollama 或 LM Studio 在本地运行模型——无需 API Key。
...
xclaw models remove-local <model_id> --yes   # 跳过确认
```
...
xClaw 集成 Ollama 以在本地运行模型。模型从 Ollama 守护进程动态加载——请先从 [ollama.com](https://ollama.com) 安装 Ollama。

安装 Ollama SDK：`pip install 'xclaw[ollama]'`（或使用 `--extras ollama` 重新运行安装脚本）
...
xclaw models config           # 选择 Ollama → 从模型列表中选择
xclaw models set-llm          # 切换到其他 Ollama 模型
```

**与本地模型的主要区别：**

- 模型来自 Ollama 守护进程（不由 xClaw 下载）
- 使用 `ollama` 命令管理模型（非 `xclaw models`）
- 通过 Ollama CLI 或 xClaw 添加/删除模型时，模型列表自动更新

> **注意：** API Key 的有效性需要用户自行保证，xClaw 不会验证。
...
| 命令                      | 说明                 |
| ------------------------- | -------------------- |
| `xclaw env list`          | 列出所有已配置的变量 |
| `xclaw env set KEY VALUE` | 设置或更新变量       |
| `xclaw env delete KEY`    | 删除变量             |
...
> **注意：** xClaw 只负责存储和加载，值的有效性需要用户自行保证。
...
**别名：** 可以用 `xclaw channel`（单数）作为 `xclaw channels` 的简写。

| 命令                           | 说明                                                                            |
| ------------------------------ | ------------------------------------------------------------------------------- |
| `xclaw channels list`          | 查看所有频道的状态（密钥脱敏）                                                  |
| `xclaw channels send`          | 向用户/会话单向发送消息（需要全部 5 个参数）                                    |
| `xclaw channels install <key>` | 在 `custom_channels/` 安装频道：创建模板，或用 `--path` / `--url` 安装          |
| `xclaw channels add <key>`     | 安装并加入 config；内置频道只写 config；支持 `--path` / `--url`                 |
| `xclaw channels remove <key>`  | 从 `custom_channels/` 删除自定义频道（内置不可删）；`--keep-config` 保留 config |
| `xclaw channels config`        | 交互式启用/禁用频道并填写凭据                                                   |

**多智能体支持：** 所有命令都支持 `--agent-id` 参数（默认为 `default`）。

```bash
xclaw channels list                    # 看默认智能体的频道状态
xclaw channels list --agent-id abc123  # 看特定智能体的频道状态
xclaw channels install my_channel      # 创建自定义频道模板
xclaw channels install my_channel --path ./my_channel.py
xclaw channels add dingtalk            # 把钉钉加入 config
xclaw channels remove my_channel       # 删除自定义频道（并默认从 config 移除）
xclaw channels remove my_channel --keep-config   # 只删模块，保留 config 条目
xclaw channels config                  # 交互式配置默认智能体
xclaw channels config --agent-id abc123 # 交互式配置特定智能体
```
...
使用 `xclaw channels send` 主动向用户/会话推送消息，支持所有已配置的频道。这是**单向发送** —— 不会返回回复。
...
- `--target-user`：用户 ID（从 `xclaw chats list` 获取）
- `--target-session`：会话 ID（从 `xclaw chats list` 获取）
- `--text`：消息内容

**重要提示：**

- 发送前必须先用 `xclaw chats list` 查询 —— 不要猜测 `target-user` 或 `target-session`
- 如果有多个会话，优先使用最近更新的
- 这仅用于主动通知；智能体间通信请用 `xclaw agents chat`（见下方"智能体"章节）

**与 `xclaw agents chat` 的区别：**

- `xclaw channels send`：智能体向用户/频道推送，单向，无回复
- `xclaw agents chat`：智能体间通信，双向，有回复
...
智能体通过启用 **multi_agent_collaboration** 技能，可以在需要时自动使用 `xclaw agents chat` 与其他智能体协作。

**别名：** 可以用 `xclaw agent`（单数）作为 `xclaw agents` 的简写。

| 命令                | 说明                                             |
| ------------------- | ------------------------------------------------ |
| `xclaw agents list` | 列出所有已配置的智能体（ID、名称、描述、工作区） |
| `xclaw agents chat` | 与另一个智能体通信（双向，支持多轮对话）         |
...
**与 `xclaw channels send` 的区别：**

- `xclaw agents chat`：智能体间，双向，返回回复
- `xclaw channels send`：智能体到用户/频道，单向，无回复

---

## 定时任务

让 xClaw 按时间自动执行任务——「每天 9 点发消息」「每 2 小时提问并转发回复」。
**需要 `xclaw app` 正在运行。**
...
| 命令                         | 说明                           |
| ---------------------------- | ------------------------------ |
| `xclaw cron list`            | 列出所有任务                   |
| `xclaw cron get <job_id>`    | 查看任务配置                   |
| `xclaw cron state <job_id>`  | 查看运行状态（下次运行时间等） |
| `xclaw cron create ...`      | 创建任务                       |
| `xclaw cron delete <job_id>` | 删除任务                       |
| `xclaw cron pause <job_id>`  | 暂停任务                       |
| `xclaw cron resume <job_id>` | 恢复暂停的任务                 |
| `xclaw cron run <job_id>`    | 立刻执行一次                   |
...
JSON 结构见 `xclaw cron get <job_id>` 的返回。
...
通过 API 管理聊天会话。**需要 `xclaw app` 正在运行。**

### xclaw chats

| 命令                                   | 说明                                               |
| -------------------------------------- | -------------------------------------------------- |
| `xclaw chats list`                     | 列出所有会话（支持 `--user-id`、`--channel` 筛选） |
| `xclaw chats get <id>`                 | 查看会话详情和消息历史                             |
| `xclaw chats create ...`               | 创建新会话                                         |
| `xclaw chats update <id> --name "..."` | 重命名会话                                         |
| `xclaw chats delete <id>`              | 删除会话                                           |

**多智能体支持：** 所有命令都支持 `--agent-id` 参数（默认为 `default`）。

```bash
xclaw chats list                        # 默认智能体的会话
xclaw chats list --agent-id abc123      # 特定智能体的会话
...
xclaw chats update <chat_id> --name "新名称"
...
清空工作目录（默认 `~/.xclaw`）下的所有内容。

```bash
xclaw clean             # 交互确认
xclaw clean --yes       # 不确认直接清空
xclaw clean --dry-run   # 只列出会被删的内容，不删
```
...
| 选项            | 默认值      | 说明                                      |
| --------------- | ----------- | ----------------------------------------- |
| `--host`        | `127.0.0.1` | API 地址（自动检测上次 `xclaw app` 的值） |
| `--port`        | `8088`      | API 端口（自动检测上次 `xclaw app` 的值） |
| `-h` / `--help` |             | 显示帮助                                  |
...
| `xclaw init`     | —                                                                                    |        否         |
| `xclaw app`      | —                                                                                    | —（启动服务本身） |
| `xclaw models`   | `list` · `config` · `config-key` · `set-llm` · `download` · `local` · `remove-local` |        否         |
| `xclaw env`      | `list` · `set` · `delete`                                                            |        否         |
| `xclaw channels` | `list` · `send` · `install` · `add` · `remove` · `config`                            |      **是**       |
