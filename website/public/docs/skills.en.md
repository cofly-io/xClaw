...
- **Working directory:** Edit skill files directly under `$QWENPAW_WORKING_DIR`
  (default `~/.xclaw`), including `$QWENPAW_WORKING_DIR/skill_pool/` and each
...
- **Conflict handling:** If save, import, upload, or broadcast would land on a
  name that already exists, xClaw returns a conflict instead of silently
...
1. **Import built-ins**.
   ...
   | **QA_source_index**           | Internal xClaw source/doc index skill for quickly mapping keywords to source paths and local docs.                            | Built-in                                                       |
   | **cron**                      | Scheduled jobs. Create, list, pause, resume, or delete jobs via `xclaw cron` or Console **Control → Cron Jobs**.              | Built-in                                                       |
   ...
   | **guidance**                  | Answer xClaw installation and configuration questions by consulting local docs first.                                         | Built-in                                                       |
   ...
   **out-of-date**. Use **Update Built-in Skills** to add missing built-ins
   ...
   [CLI](./cli) (`xclaw cron`) or Console **Control → Cron Jobs**:

   - Create: `xclaw cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
   - List: `xclaw cron list`
   - Check state: `xclaw cron state <job_id>`
...
`$QWENPAW_WORKING_DIR/workspaces/{agent_id}/skills/`, including using xClaw itself to help
...
variables, declare them in `metadata.requires`; xClaw exposes them as
...
workspace and channel, xClaw injects that config into the runtime environment
...
For `requires` metadata, the parser checks keys in order: `metadata.openclaw.requires` → `metadata.xclaw.requires` → `metadata.requires`. The first one found is used.
