...
xClaw defines an agent's "persona" through a set of Markdown files that are loaded into the system prompt. These files determine the agent's behavioral style, working approach, and personality traits. By editing these files, you can shape the agent into your ideal assistant—whether that's a meticulous work aide, a warm life companion, or a technical expert.

---

## Persona Files

Agent persona files are Markdown documents stored in the agent's workspace directory. The workspace location is determined by the `QWENPAW_WORKING_DIR` environment variable (defaults to `~/.xclaw`), with the full path being:
...
When running `xclaw init` for the first time, BOOTSTRAP.md is automatically created to guide the initial "conversation" between user and agent, establishing identity, preferences, and style. Once complete, the agent writes the configuration to PROFILE.md and SOUL.md, then deletes BOOTSTRAP.md.
...
You can also directly edit the `system_prompt_files` field in the agent configuration file (`~/.xclaw/workspaces/{agent_id}/agent.json`) to manage persona file loading:
...
Running `xclaw init` automatically creates template files based on your chosen language (`zh` / `en` / `ru`):
...
If using `xclaw init --defaults`, the default language is `zh` (Chinese).
...
xClaw automatically creates a built-in agent named **"QA Agent"** (ID: `xClaw_QA_Agent_0.2`) when you first run `xclaw init`.

### QA Agent Features

This is an agent **specifically designed to answer xClaw-related questions**:

- **Dedicated persona**: Uses persona files optimized for Q&A (different from regular agents)
- **Pre-installed skills**: Auto-enables `guidance` and `QA_source_index` skills for querying xClaw documentation and source code
...
- **Auto-maintained**: Running `xclaw init` ensures this agent exists

### How to Use?

You can select "QA Agent" in the agent switcher at the top-right of the Console, then ask it any questions about xClaw.
...
- **Can delete**: Delete it in "Settings → Agent Management" (doesn't affect other agents; will be recreated on next `xclaw init`)
- **Workspace location**: `$QWENPAW_WORKING_DIR/workspaces/xClaw_QA_Agent_0.2/` (defaults to `~/.xclaw/workspaces/xClaw_QA_Agent_0.2/`)
