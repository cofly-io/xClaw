...
xClaw 对 **ACP（Agent Client Protocol）** 提供两种互补的支持方式：

1. **xClaw 将 ACP 作为 Tool 使用**：xClaw 连接外部 ACP runner，并将其作为委托协作能力使用
2. **xClaw 作为 ACP Server**：外部客户端通过 ACP 连接到 xClaw

本页会同时介绍这两种模式，以及各自适合的使用场景。

---

## xClaw 将 ACP 作为 Tool 使用

在这种模式下，xClaw 会作为 **ACP client / orchestrator**，连接**已配置并启用的外部 ACP runner**，并将它们接入当前会话，作为委托协作能力使用。

这类能力的实际调用入口是内置工具 `delegate_external_agent`。它适用于你希望 xClaw 与其他支持 ACP 的外部 agent runtime 协作的场景，例如源码中默认内置的 `opencode`、`qwen_code`、`claude_code`、`codex`。这些 agent 可参考 ACP 官方的 Agent 列表与接入说明：<https://agentclientprotocol.com/get-started/agents>。换句话说，xClaw 不是“直接和任意外部 agent 交互”，而是通过 ACP 配置中已注册的 runner，在会话内发起、继续、响应和关闭一次委托式协作。

### 这种模式能做什么

在这种模式下，xClaw 会通过内置的 `delegate_external_agent` 工具来：
...
从概念上看，这让 xClaw 可以把一个外部 agent 当作“可协作的工具能力”来使用，同时仍然由 xClaw 负责主会话编排。
...
命令行侧准备完成后，你可以在 xClaw 中配置自定义 runner，或直接使用内置 runner 与其协作。
...
当外部 ACP runner 请求权限时，xClaw **不会替用户做决定**。
...
这样可以让委托式 ACP 执行与 xClaw 其他能力保持一致的用户可控安全模型。

### 什么时候使用 ACP as Tool

以下场景适合使用这种模式：

- 你希望 xClaw 与另一个 agent runtime 协作
- 你有一个专门处理某类任务的 ACP-compatible 外部 runner
- 你希望由 xClaw 作为主控编排者，把部分工作委托给外部 agent

### ACP Tool 与 MCP 的区别

ACP as Tool 和 MCP 解决的问题并不相同：

- **MCP**：让 xClaw 连接外部服务和工具服务器
- **ACP as Tool**：让 xClaw 连接外部 **agent** runtime
...
在这种模式下，xClaw 会通过 stdio JSON-RPC 将自己暴露为一个符合 [Agent Client Protocol (ACP)](https://github.com/agentclientprotocol/python-sdk) 规范的智能体服务。外部客户端，如 [Zed](https://zed.dev)、[OpenCode](https://github.com/nicholasgasior/opencode) 或任何兼容 ACP 的编辑器，都可以通过 `xClaw acp` 命令连接到 xClaw，并以编程方式与之交互。
...
2. **WORKING_DIR 配置**：从 `WORKING_DIR` 内的 `config.json` 中读取 `agents.active_agent`（默认 `~/.xClaw`，旧版安装为 `~/.xClaw`；可通过 `xClaw_WORKING_DIR` 环境变量覆盖）
...
| 维度         | xClaw as ACP Server            | xClaw using ACP as Tool           |
| ------------ | ------------------------------ | --------------------------------- |
| xClaw 的角色 | Server / 被连接的智能体        | Client / 编排者                   |
| 连接方向     | 外部客户端连接 xClaw           | xClaw 连接外部 runner             |
| 主要目的     | 让编辑器或外部客户端驱动 xClaw | 让 xClaw 把工作委托给另一个 agent |
| 典型入口     | `xClaw acp`                    | delegation tool + ACP runner 配置 |
...
ACP 在 xClaw 中并不是单一能力，而是支持两个方向：

- **向外暴露 xClaw**：作为 ACP server
- **从 xClaw 向外协作**：把外部 ACP agent 当作委托工具使用

如果你是要把 xClaw 接入另一个客户端，优先看 **ACP Server**。
如果你是希望 xClaw 去协调另一个 agent runtime，优先看 **ACP as Tool**。
