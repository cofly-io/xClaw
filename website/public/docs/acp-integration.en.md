...
xClaw supports **ACP (Agent Client Protocol)** in two complementary ways:

1. **xClaw using ACP as a Tool**: xClaw connects to external ACP runners and uses them as delegated collaborators
2. **xClaw as an ACP Server**: external clients connect to xClaw over ACP

This page explains both modes and the scenarios each one fits best.

---

## xClaw using ACP as a Tool

In this mode, xClaw acts as an **ACP client / orchestrator**, connecting to **configured and enabled external ACP runners** and bringing them into the current session as delegated collaboration capabilities.

The actual entry point for this mode is the built-in tool `delegate_external_agent`. It is intended for scenarios where xClaw needs to collaborate with other ACP-capable external agent runtimes, such as the built-in examples `opencode`, `qwen_code`, `claude_code`, and `codex`. For more ACP-compatible agents, see the official ACP agent list and integration guide: <https://agentclientprotocol.com/get-started/agents>. In other words, xClaw does not directly talk to arbitrary external agents. It talks to runners that have been registered in ACP configuration, and it starts, continues, responds to, and closes delegated collaboration sessions through them.

### What this mode does

In this mode, xClaw uses the built-in `delegate_external_agent` tool to:
...
Conceptually, this lets xClaw treat an external agent as a collaborative, tool-like capability while keeping xClaw as the primary orchestrator of the main conversation.
...
Once the command-line side is ready, you can configure a custom runner in xClaw or collaborate with one of the built-in runners directly.
...
When an external ACP runner asks for permission, xClaw does **not** decide on the user's behalf.
...
This keeps delegated ACP execution aligned with the same user-controlled safety model used elsewhere in xClaw.

### When to use ACP as a Tool

Use this mode when:

- you want xClaw to collaborate with another agent runtime
- you have a specialized ACP-compatible external runner for a certain class of tasks
- you want xClaw to remain the primary orchestrator while delegating part of the work outward

### ACP Tool vs MCP

ACP as a Tool and MCP solve different problems:

- **MCP** connects xClaw to external services and tool servers
- **ACP as a Tool** connects xClaw to an external **agent** runtime
...
## xClaw as an ACP Server

In this mode, xClaw exposes itself as an [Agent Client Protocol (ACP)](https://github.com/agentclientprotocol/python-sdk) compliant agent service over stdio JSON-RPC. External clients, such as [Zed](https://zed.dev), [OpenCode](https://github.com/nicholasgasior/opencode), or any ACP-compatible editor, can connect to xClaw via the `xClaw acp` command and interact with it programmatically.

### Quick Start

```bash
# Start xClaw as an ACP agent
xClaw acp

# Use a specific agent profile
xClaw acp --agent mybot

# Use a custom workspace directory
xClaw acp --workspace /path/to/workspace
...
ACP in xClaw is not just one feature. It supports both directions:

- **Expose xClaw outward** as an ACP server
- **Reach outward from xClaw** to external ACP agents as delegated tools

If you are integrating xClaw into another client, start with **ACP Server**.
If you want xClaw to coordinate with another agent runtime, use **ACP as a Tool**.
