...
xClaw uses **MCP (Model Context Protocol)** to connect to external services and provides a suite of **built-in tools** that enable agents to access filesystems, execute commands, browse the web, and more.

---

## Concepts

xClaw provides two types of tools for agents:

1. **Built-in Tools**: Ready-to-use tools provided by xClaw core, such as file operations, command execution, and browser automation
...
**MCP (Model Context Protocol)** allows xClaw to connect to external MCP servers, extending the agent's ability to access filesystems, databases, APIs, and other external resources.
...
xClaw supports three JSON formats—choose one:
...
xClaw provides a set of ready-to-use built-in tools that agents can directly call to perform various tasks.
