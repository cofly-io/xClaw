...
**频道** = 你和 xClaw 在「哪里」对话：接钉钉就在钉钉里回，接 QQ 就在 QQ 里回。不熟悉这个词的话可以先看 [项目介绍](./intro)。

配置频道有两种方式：

- **控制台**（推荐）— 在 [控制台](./console) 的 **Control → Channels** 页面，点击频道卡片，在抽屉里启用并填写鉴权信息，保存即生效。
- **手动编辑 `agent.json`** — 在智能体工作区的 `agent.json` 中（如 `~/.xclaw/workspaces/default/agent.json`），将需要的频道设 `enabled: true` 并填好鉴权信息；保存后自动重载，无需重启。
...
7. （可选） **将服务器 IP 加入白名单** — 调用钉钉开放平台 API（如下载用户发送的图片和文件）时需要此配置。在应用设置中进入 **"安全设置→服务器出口 IP"**，添加运行 xClaw 的机器的公网 IP。可在终端执行 `curl ifconfig.me` 查看公网 IP。若未配置白名单，图片和文件下载将报 `Forbidden.AccessDenied.IpNotInWhiteList` 错误。

### 绑定应用

可以在console前端配置，或者修改智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）。
...
在智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）里找到 `channels.dingtalk`，填入对应信息：
...
> - 群聊场景建议显式配置 `robot_code`；留空时 xClaw 会回退使用 `client_id`。

保存后若服务已运行会自动重载；未运行则执行 `xclaw app` 启动。
...
> 注：**操作顺序**为先配置 App ID/Secret → 启动 `xclaw app` → 再在开放平台配置长连接，如果此处仍显示错误，尝试先暂停 xClaw 服务并重新启动 `xclaw app`。
...
在智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）中找到`channels.feishu`，只需填 **App ID** 和 **App Secret**（在开放平台「凭证与基础信息」里复制）：
...
> 注: **App ID** 和 **App Secret** 信息也可以在Console前端填写，但需重启 xClaw 服务，才能继续配置长链接的操作。
...
> **获取用户昵称（推荐）**：若希望会话和日志中显示**用户昵称**（如「张三#1d1a」）而非「unknown#1d1a」，需额外开通通讯录只读权限 **以应用身份读取通讯录**（`contact:user.base:readonly`）。未开通时，飞书仅返回 open_id 等身份字段，不返回姓名，xClaw 无法解析昵称。开通后需重新发布/更新应用版本，权限生效后即可正常显示用户名称。
...
3. 为了使 iMessage 中的信息能被获取，需要 **终端** （或你用来运行 xClaw 的 app） 和 **消息** 有 **完全磁盘访问权限**（系统设置 → 隐私与安全性 → 完全磁盘访问权限）。
...
4. 填写 iMessage 数据库路径。默认路径为 `~/Library/Messages/chat.db`，若你改过系统路径，请填实际路径。有以下两种填写方案：
   ...
   - 填写智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）：
...
在智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）里找到 `channels.discord`，填入对应信息：
...
5. 在**开发管理**中获取**AppID**和**AppSecret**（即 ClientSecret），填入 `agent.json`，方式见下方填写 agent.json。在**IP白名单**中添加一个IP。
   ...
   > **提示：** 如果使用魔搭创空间部署xClaw，QQ频道的IP白名单应填写：`47.92.200.108`
...
在智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）里找到 `channels.qq`，把上面两个值分别填进 `app_id` 和 `client_secret`：
...
**OneBot** 渠道通过**反向 WebSocket** 将 xClaw 连接到 [NapCat](https://github.com/NapNeko/NapCatQQ)、[go-cqhttp](https://github.com/Mrs4s/go-cqhttp)、[Lagrange](https://github.com/LagrangeDev/Lagrange.Core) 或其他任何兼容 [OneBot v11](https://github.com/botuniverse/onebot-11) 的实现。

与内置 QQ 渠道（使用官方 QQ Bot API，功能受限）不同，OneBot v11 提供**完整 QQ 协议**支持：个人号、群聊无需 @、富媒体消息等。

### 工作原理

xClaw 启动一个 WebSocket 服务器，OneBot 实现（如 NapCat）作为客户端连接过来：

```
NapCat  ──反向 WS──▶  xClaw (:6199/ws)
```
...
3. 进入 **网络配置** → **新建** → **WebSocket 客户端**（反向 WS）：
   ...
   - Access Token：与 xClaw 配置中的 `access_token` 保持一致（可选）
...
> **Docker Compose 提示：** xClaw 和 NapCat 一起用 Docker Compose 部署时，NapCat 的反向 WS 地址填 `ws://xclaw:6199/ws`（使用服务名）。
...
> **提示：** 语音和视频在渠道层已正确接收，但需要配置 xClaw 的转写服务（`transcription_provider_type`）才能让 LLM 理解内容。未配置时语音消息显示为占位符。
...
**方法二**在 `agent.json` 填写（如 `~/.xclaw/workspaces/default/agent.json`）
...
- **登录方式**：首次使用时扫描二维码授权，Token 自动持久化到本地文件（默认 `~/.xclaw/weixin_bot_token`），后续启动无需重复扫码。
- **消息接收**：通过 HTTP 长轮询（`getupdates`）持续拉取新消息，支持文本、图片、语音（ASR 转录）和文件。
- **消息发送**：通过 `sendmessage` 接口回复用户，当前仅支持文本（iLink API 限制）。

### 扫码登录（推荐通过 Console）

1. 在 xClaw Web Console 中进入 **设置 → 通道 → 微信个人（iLink）**。
...
也可直接在智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）中配置：
...
在智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）里找到 `channels.telegram`，填入对应信息：
...
3. 在控制台或智能体工作区的 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）中配置 **URL** 和 **Token**。
...
2. 模糊匹配订阅和自动推送
   ...
   模糊订阅全server/+/up主题，根据客户端的client_id自动推送到对应的主题，例如客户端向`/server/client_a/up`推送xClaw处理完后，将会向`/client/client_b/down`推送消息。
...
3. 重定向主题推送
   ...
   消息会根据redirect_client_id属性，推送至 `client/client_b/down`，从而实现跨主题推送。在物联网场景，可以做到以xClaw为核心，根据个人需求，多设备间自主推送消息。

---

## Matrix

Matrix 频道通过 [matrix-nio](https://github.com/poljar/matrix-nio) 库将 xClaw 接入任意 Matrix 服务器，支持私聊和群聊房间中的文本消息收发。
...
在 `agent.json`（如 `~/.xclaw/workspaces/default/agent.json`）中找到 `channels.matrix`：
...
Voice 频道通过 Twilio ConversationRelay 实现电话语音交互，支持语音转文本（STT）、文本转语音（TTS），让用户可以直接拨打电话与 xClaw 对话。

### 前置要求

1. **Twilio 账号**：从 [Twilio 官网](https://www.twilio.com/) 注册账号并获取凭证
2. **Cloudflare Tunnel**（或其他内网穿透方案）：将本地 xClaw 服务暴露到公网，供 Twilio 回调使用
...
Twilio 需要通过公网回调 xClaw 的 Webhook 接口，因此需要将本地服务暴露到公网。
...
配置完成后，直接拨打你购买的 Twilio 电话号码，即可与 xClaw 进行语音对话：

1. 拨打电话
2. 听到欢迎语后开始说话
3. xClaw 将语音转文本，调用 Agent 处理
...
- **目录**：工作目录下的 `custom_channels/`（默认 `~/.xclaw/custom_channels/`）用于存放自定义渠道模块。Manager 启动时会扫描该目录下的 `.py` 文件与包（含 `__init__.py` 的子目录），加载其中的 `BaseChannel` 子类，并按类的 `channel` 属性注册。
- **安装**：`xclaw channels install <key>` 会在 `custom_channels/` 下生成名为 `<key>.py` 的模板文件，可直接编辑实现；也可用 `--path <本地路径>` 或 `--url <URL>` 从本地/网络复制渠道模块。`xclaw channels add <key>` 等价于安装后并写入 config 默认项，且可加 `--path`/`--url`。
- **删除**：`xclaw channels remove <key>` 会从 `custom_channels/` 中删除该渠道模块（仅支持自定义渠道，内置渠道不可删）；加 `--no-keep-config`（默认）会同时从 `config.json` 的 `channels` 中移除对应 key。
- **Config**：`ChannelConfig` 使用 `extra="allow"`，`config.json` 的 `channels` 下可写任意 key；自定义渠道的配置会保存在 extra 中。配置方式与内置一致：`xclaw channels config` 交互式配置，或直接编辑 config。

### HTTP 路由注册

对于需要 Webhook 回调的渠道（如微信、Slack、LINE 等），可以通过在模块中导出 `register_app_routes` 可调用对象来注册自定义 HTTP 路由，无需修改 xClaw 核心源码。

xClaw 启动时会扫描 `custom_channels/` 下的模块，发现 `register_app_routes` 后将其与 FastAPI `app` 实例一起调用，渠道即可注册所需的任何路由。
...
**实际案例**：微信 ClawBot 集成（[PR #2140](https://github.com/agentscope-ai/xClaw/pull/2140)、[Issue #2043](https://github.com/agentscope-ai/xClaw/issues/2043)）通过此机制注册 `/api/wechat/callback` 路由，使用腾讯官方 SDK 处理消息投递。
