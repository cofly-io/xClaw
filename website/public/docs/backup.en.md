...
**Backup & Restore** lets you create snapshots of your xClaw instance: visually create, export, import, and restore your entire agent environment. It is designed for scenarios such as **rolling back before a version upgrade, migrating between machines, or keeping a safety net before risky changes**.
...
| **Secrets**          | `~/.xclaw.secret/`                | **LLM provider configuration (including API keys)**, plus environment variables used by tools and skills.                                                                             |
...
A backup ID has the format `xclaw-<version>-<timestamp>-<short8>`, which makes it easy to identify the source version and creation time across machines.
...
2. Tick what you need:
   ...
   - **Secrets**: whether to include the secrets (i.e. the `~/.xclaw.secret/` directory). Off by default; turning it on shows the same red sensitive-content warning.
...
When you click the **Restore** button on any backup row, xClaw first opens the **Pre-restore backup** dialog:
...
- **Pick agents one by one**: restore only the agents you tick; agents outside the restore scope are kept as they are. You can also specify the default storage location for newly added agents at restore time; if unspecified, they go to `~/.xclaw/workspaces/<agent_id>/`.
...
Add `-v xclaw-backups:/app/working.backups` to your `docker run` command:
...
A: xClaw shows an overwrite confirmation; confirm it to continue importing and overwrite the existing backup.
