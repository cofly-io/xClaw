...
**控制台** 是 xClaw 内置的 Web 管理界面。运行 `xclaw app` 后，在浏览器中打开
...
这是你和 xClaw 对话的地方。打开控制台后默认就是这个页面。
...
**发送消息：**
在底部输入框中输入内容，按 **Enter** 或点击发送按钮（↑），xClaw 会实时回复。
...
在这里创建和管理 xClaw 按时间自动执行的定时任务。

![定时任务](https://img.alicdn.com/imgextra/i1/O1CN01UqMEHK1g2WfLs2wUQ_!!6000000004084-2-tps-3822-2070.png)

**创建新任务：**

> 如果定时任务没有创建成功，可以参考 [FAQ](https://xclaw.agentscope.io/docs/faq) 的 **定时任务错误排查** 寻找原因

创建定时任务的 **最简单的方式是直接与 xClaw 对话**，让他为你创建。例如你想在钉钉上收到喝水提醒，则在钉钉上与 xClaw 对话：“帮我创建一个定时任务，每隔 5 分钟提醒我喝水。”创建完成后，可以在控制台的定时任务页面看到创建好的定时任务。
...
2. 按区域填写表单：
   ...
   - **任务类型及内容**
     ...
     - 选择**agent**：填写**请求内容**，会定时向xClaw转发content.text中的请求文本
...
为**当前选中的智能体**配置定时「自检」：按间隔把 `HEARTBEAT.md` 里的内容当作用户消息发给 xClaw，并可把回复投递到指定目标。
...
在这里编辑定义 xClaw 人设和行为的文件——SOUL.md、AGENTS.md、
HEARTBEAT.md 等——全部在浏览器中完成。

> **多智能体：** 从 **v0.1.0** 开始，xClaw 支持**多智能体**功能。
> 您可以在同一个 xClaw 实例中运行多个独立的智能体，每个智能体拥有独立的
...
在这里管理扩展 xClaw 能力的技能（如读取 PDF、创建 Word 文档、获取新闻等），更详细的内容请看 [Skills](./skills)。
...
在这里管理 xClaw 的工具和技能在运行时需要的环境变量（如 `TAVILY_API_KEY`）。
...
> **注意：** 环境变量值的有效性需要用户自行保证，xClaw 只负责存储和加载。
>
...
在聊天中直接问 xClaw「最近用了多少 token？」或「帮我看看 token 消耗」，Agent 会调用 `get_token_usage` 工具并返回统计结果。

> 数据存储在 `~/.xclaw/token_usage.json`，可通过 `QWENPAW_TOKEN_USAGE_FILE` 环境变量自定义文件名。详见 [配置 — 环境变量](./config#环境变量)。
...
- **转写后端**：**关闭**；**Whisper API** — 使用兼容 OpenAI `audio/transcriptions` 的提供商，需在 [模型](#模型) 中配置好对应密钥并在此选中提供商；**本地 Whisper** — 本机运行，需安装 `ffmpeg` 与 `pip install 'xclaw[whisper]'`。
