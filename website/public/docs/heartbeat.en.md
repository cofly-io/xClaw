...
In xClaw, **heartbeat** means: on a fixed interval, ask xClaw the
“questions” you wrote in a file, and optionally send the xClaw’s reply to
**the channel where you last chatted**. Good for “regular check-ins, daily
digests, scheduled reminders” — xClaw runs without you sending a
message.
...
1. In the current agent’s workspace there is a **heartbeat query file** (default
   ...
   content is **what to ask xClaw on each run** (one or more paragraphs; xClaw
...
2. When **`enabled` is true** in config, the system runs on your **every**
   ...
   the user message → xClaw replies.
3. **Whether the reply goes to a channel** is set by **target**:
   - **main** — Run xClaw only; don’t send the reply to any channel (e.g. local
     self-check, logs).
   - **last** — Send the reply to the **channel/session where you last talked
     to xClaw** (e.g. if you last used DingTalk, the heartbeat reply goes to
     DingTalk).
...
Default `QWENPAW_WORKING_DIR` is `~/.xclaw` (override with **`QWENPAW_WORKING_DIR`**);
...
If you ran `xclaw init` without `--defaults`, you may be prompted to edit
...
Example (heartbeat on, xClaw only, no channel, every 30m) — in that agent’s
**`agent.json`**:
...
> [CLI](./cli) `xclaw cron create` for cron jobs, not heartbeat.
