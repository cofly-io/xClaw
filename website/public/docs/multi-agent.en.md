...
xClaw supports **multi-agent**, allowing you to run multiple independent AI agents in a single xClaw instance.
...
Simply put, **multi-agent** lets you run multiple "personas" in one xClaw, where each persona:
...
You might use xClaw across multiple platforms:
...
After starting xClaw, you'll see the **Agent Selector** in the **top-left corner** of the console:
...
**Note**: After deletion, the workspace directory is retained (to prevent accidental data loss). To completely remove it, manually delete the `~/.xclaw/workspaces/{agent_id}` directory.
...
If you previously used xClaw **v0.0.x**, upgrading to **v0.1.0** will **automatically migrate**:
...
2. **Verify Migration**
   ...
   - After starting xClaw, check the agent list in console
...
xclaw skills config --agent-id abc123
...
```
~/.xclaw/workspaces/{agent_id}/PROFILE.md
```
...
# Workspace: ~/.xclaw/workspaces/code_reviewer
...
Workspace: ~/.xclaw/workspaces/writer_bot
Profile: [Auto-generated detailed capability description]
```
...
xclaw channels list --agent-id abc123
xclaw cron list --agent-id abc123
xclaw skills list --agent-id abc123
...
- `xclaw channels` - channel management
- `xclaw cron` - cron jobs
- `xclaw daemon` - runtime status
- `xclaw chats` - chat management
- `xclaw skills` - skill management

**Commands NOT Supporting `--agent-id`** (global operations):

- `xclaw init` - initialization
- `xclaw providers` - model providers
- `xclaw models` - model configuration
- `xclaw env` - environment variables
...
cp -r ~/.xclaw/workspaces/abc123 ~/backups/agent-abc123-$(date +%Y%m%d)

# Backup all agents
cp -r ~/.xclaw/workspaces ~/backups/workspaces-$(date +%Y%m%d)
```
