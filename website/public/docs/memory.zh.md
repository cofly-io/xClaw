...
**长期记忆** 让 xClaw 拥有跨对话的持久记忆能力：通过文件工具将关键信息写入 Markdown 文件长期保存，并配合语义检索随时召回。
...
xClaw ??? **ReMeLight** ?????????? **ADBPG**?AnalyticDB for PostgreSQL??????????????????? `memory_manager_backend` ?? `adbpg` ??? `adbpg_memory_config`?
...
> ⚠️ 切换后端不支持热更新，保存后需要重启 xClaw 才能生效（页面也会以黄色横幅提醒）。
...
通过 psycopg2 直连 ADBPG 数据库，需额外安装依赖：`pip install 'xclaw[adbpg]'`。
