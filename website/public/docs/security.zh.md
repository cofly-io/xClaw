...
xClaw 内置了安全功能，保护你的 Agent 在运行过程中产生的不安全行为和不安全技能的影响。这些功能在控制台 **设置 → 安全** 中配置，也可以通过 `config.json` 进行设置。

## 概述

xClaw 的安全系统由三个核心安全层组成:
...
**默认保护**: `{WORKING_DIR}.secret/` 目录(存储 API 密钥、认证凭据和提供商配置)默认包含在敏感文件列表中。默认情况下,`WORKING_DIR` 为 `~/.xclaw/`,完整路径为 `~/.xclaw.secret/`。
...
扫描器使用 `src/xclaw/security/skill_scanner/rules/signatures/` 中的 YAML 规则文件。你可以通过 YAML 策略文件自定义扫描策略:
...
xClaw 支持可选的 Web 登录认证,保护控制台免受未授权访问。认证**默认关闭**,需要通过 `QWENPAW_AUTH_ENABLED` 环境变量显式启用。

![login](https://img.alicdn.com/imgextra/i4/O1CN01VdXCuP1tWpsl0TlQ5_!!6000000005910-2-tps-3822-2070.png)

### 工作原理

1. **启用认证** — 设置 `QWENPAW_AUTH_ENABLED=true` 并启动 xClaw
2. **注册流程**:
   - 首次访问时,控制台显示**注册页面**
   - 创建唯一的管理员账户(用户名 + 密码)
   - 系统采用单用户模式,专为个人使用设计
...
4. **自动注册**(可选):
   ...
   - xClaw 启动时自动创建管理员账户,跳过网页注册
   - 适用于 Docker、Kubernetes、服务器管理面板等自动化部署场景
5. **本地免认证** — 来自本地(`127.0.0.1` / `::1`)的请求自动跳过认证,CLI 命令(`xclaw app`、`xclaw chat` 等)无需令牌即可正常工作
...
然后通过 `--env-file .env` 传递给 Docker，或在运行 `xclaw app` 前在 shell 中 source 该文件。
...
```bash
docker exec -it <容器名> xclaw auth reset-password
```
...
```bash
# 删除认证文件
rm ~/.xclaw.secret/auth.json  # 或 $WORKING_DIR.secret/auth.json
# 重启 xClaw,下次访问时重新注册
xclaw app
```
