...
### xClaw 与 OpenClaw 的功能对比

请查看 [对比](/docs/comparison) 页面了解详细的功能对比。

### xClaw如何安装

xClaw 支持多种安装方式，详情请见文档 [快速开始](https://xclaw.agentscope.io/docs/quickstart)：
...
>
>    - **找到安装目录**：
>      - 检查 `uv` 是否可用：在 CMD 中输入 `uv --version` ，如果显示版本号，则**只需配置 xClaw 路径**；如果提示 `'uv' 不是内部或外部命令，也不是可运行的程序或批处理文件。`，则需同时配置两者。
>      - uv路径（任选其一，取决于安装位置，若`uv`不可用则填）：通常在`%USERPROFILE%\.local\bin`、`%USERPROFILE%\AppData\Local\uv`或 Python 安装目录下的 `Scripts` 文件夹
>      - xClaw路径：通常在 `%USERPROFILE%\.xclaw\bin` 。
>    - **手动添加到系统的 Path 环境变量**：
...
> - **配置`uv`环境变量**：将`uv`所在目录和 `%USERPROFILE%\.xclaw\bin` 添加到系统的 `Path` 变量中。
> - **重新运行**：打开新终端，再次执行安装脚本以完成 `xClaw` 安装。
> - **配置`xClaw`环境变量**：将 `%USERPROFILE%\.xclaw\bin` 添加到系统的 `Path` 变量中。

### xClaw如何更新

要更新 xClaw 到最新版本，可根据你的安装方式选择对应方法：
...
5. 如果你使用的是 Windows 桌面版（exe），目前需要卸载后重新安装：
   - 在电脑中卸载 xClaw
   - 下载最新版本：https://github.com/agentscope-ai/xClaw/releases
   - 重新安装

升级后重启服务 xclaw app。

原 CoPaw 用户升级 xClaw，下载最新的 xClaw 即可，无需做额外适配即可继续使用 CoPaw 时期的所有配置、记忆、技能等。
...
控制台默认地址为 `http://127.0.0.1:8088/`，使用默认配置快速初始化后，可以进入控制台快捷自定义相关内容。详情请见[快速开始](https://xclaw.agentscope.io/docs/quickstart)。

### Windows 端口 8088 冲突问题

在 Windows 上，Hyper-V 和 WSL2 可能会保留某些端口范围，这可能与 xClaw 的默认端口 **8088** 冲突。此问题影响所有安装方式（pip 安装、脚本安装、Docker、桌面应用）。
...
- xClaw 无法启动，或浏览器无法访问 `http://127.0.0.1:8088/`
...
具体版本变更可在官网 [更新日志](https://xclaw.agentscope.io/release-notes/?lang=zh) 或 xClaw GitHub 仓库 [Releases](https://github.com/agentscope-ai/xClaw/releases) 中查看。

### 如何配置模型

在控制台进入 **设置 → 模型** 中进行配置，详情请见文档 [模型](https://xclaw.agentscope.io/docs/models)：
...
命令行也可使用 `xclaw models` 系列命令完成配置、下载和切换，详情请见文档 [CLI → 模型与环境变量 → xclaw models](https://xclaw.agentscope.io/docs/cli#xclaw-models)。

### 如何使用 xClaw-Flash 系列模型

xClaw-Flash 是 xClaw 官方根据 xClaw 的应用场景专门调优的系列模型，共有 2B, 4B 和 9B 三个版本，且每个版本除原始模型外还提供了 4 bit 和 8 bit 两种量化版本，适合不同的显存环境和性能需求。

xClaw-Flash 模型目前已经在 [ModelScope](https://www.modelscope.cn/organization/AgentScope?tab=model) 以及 [Hugging Face](https://huggingface.co/agentscope-ai/models) 上开源，你可以直接从这两个平台下载使用。

xClaw 内置的本地提供商均可接入 xClaw-Flash 模型：

**xClaw Local (llama.cpp)**

直接在 xClaw Local 的模型界面中选择下载 xClaw-Flash 模型并启动即可。

![Start Model](https://img.alicdn.com/imgextra/i2/O1CN01Nl0aQb1a3XqqosqAC_!!6000000003274-2-tps-1342-1682.png)

> xClaw Local 目前仍处于测试阶段，对不同设备的兼容性以及运行稳定性仍在持续优化中，如果你在使用过程中遇到任何问题，欢迎随时在 GitHub 上提 issue 反馈。
> 如果无法正常使用 xClaw Local，建议先使用 Ollama 或 LM Studio 部署 xClaw-Flash 模型。

**Ollama**:

1. 从 [ModelScope](https://www.modelscope.cn/organization/AgentScope?tab=model) 或 [Hugging Face](https://huggingface.co/agentscope-ai/models) 下载 xClaw-Flash 量化版模型，这些模型后缀为 `Q8_0` 或 `Q4_K_M`，例如 [xClaw-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/xClaw-Flash-4B-Q4_K_M)。
...
创建一个包含以下内容的文本文件 `xclaw-flash.txt`，注意将 `/path/to/your/xclaw-xxx.gguf` 替换为你下载的 xClaw-Flash 模型仓库中 `.gguf` 文件的绝对路径：
...
4. 在 xClaw 的模型配置中选择 Ollama 提供商，并在模型页面中自动获取模型即可。

**LM Studio**:

1. 参考 Ollama 的步骤 1 下载合适的 xClaw-Flash 量化版模型。
...
4. 在 xClaw 的模型配置中选择 LM Studio 提供商，并在模型页面中自动获取模型即可。

### 使用 Ollama / LM Studio 部署的模型时，为什么 xClaw 无法完成多轮交互、复杂工具调用，或记不住之前的指令？

这类问题通常不是 xClaw 本身异常，而是**模型上下文长度配置过小**导致的。

当你使用 Ollama 或 LM Studio 部署本地模型时，如果模型的 `context length` 设置太低，xClaw 在以下场景中就可能表现异常：
...
- 运行 xClaw 前，请将模型的 `context length` 设置为**至少 32K**
- 如果任务较复杂、工具调用较多或对话轮次较长，实际可能需要设置到**高于 32K**

> ⚠️ **运行 xClaw 前必须将上下文长度设为 32K 以上**
>
> 对于 Ollama 和 LM Studio 部署的本地模型，如果要让 xClaw 正常完成多轮交互、复杂工具调用和长上下文任务，通常必须提供 **32K 或更高** 的上下文长度；在更复杂的场景下，可能还需要进一步提高。
>
...
最方便的定时任务创建方式是，在你想要获取定时任务返回结果的频道，与xClaw对话，让xClaw帮你创建一个定时任务。例如，可以直接与xClaw对话：“帮我创建一个定时任务，每隔五分钟提醒我喝水。”之后可以在控制台中看到状态为已启用的定时任务。

如果定时任务没有正常启动，可以按照以下几个步骤排查：

1. 首先确认 xClaw 服务是在正常运行中的。
...
6. 排查结束后，如果想确认一下定时任务是否创建成功，且能成功触发，可以点击 **立即执行**，若成功创建，则可在对应频道收到回复。或者也可以直接与 xClaw 对话：“帮我触发一下刚刚创建的提醒喝水定时任务”。

   ![exec](https://img.alicdn.com/imgextra/i3/O1CN01a1IIsY1PhQZ5YXlCe_!!6000000001872-2-tps-3232-890.png)

### 如何管理Skill

进入控制台 **智能体 → 技能**，可以启用/禁用技能、创建自定义技能、以及从 Skills Hub 中导入技能。详情请见文档 [Skills](https://xclaw.agentscope.io/docs/skills)。

### 如何配置MCP

进入控制台 **智能体 → MCP**，进行 MCP 客户端的启用/禁用/删除/创建，详情请见文档 [MCP](https://xclaw.agentscope.io/docs/mcp)。
...
xClaw 支持百炼 Coding Plan 获取的 API key。如果仍报错，请重点检查：
...
为了加快修复与排查，共建良好社区生态，建议遇到报错时，首选在 xClaw 的 GitHub 仓库中提 [issue](https://github.com/agentscope-ai/xClaw/issues)，请附上完整报错信息，并上传错误详情文件。
...
请将后面的`/var/folders/.../qwenpaw_query_error_qzbx1mv1.json`文件一并上传，同时提供你当前的模型提供商、模型名和 xClaw 的具体版本。
