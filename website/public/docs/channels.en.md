...
A **channel** is where you talk to xClaw: connect DingTalk and it replies
...
- **Edit `agent.json` directly** — Agent workspace config at `~/.xclaw/workspaces/{agent_id}/agent.json`, set `enabled: true` and fill in that platform's credentials. Saving triggers a reload without restarting the app.
...
In your agent's `agent.json` (e.g., `~/.xclaw/workspaces/default/agent.json`), find `channels.dingtalk` and fill in the corresponding information, for example:
...
> - `robot_code` is recommended in group scenarios; if empty, xClaw falls back to `client_id`.

Save the file; if the app is already running, the channel will reload. Otherwise run `xclaw app`.
...
4. Run **`xclaw app`** to start xClaw
...
> **Note:** Follow this order: Configure App ID/Secret → start `xclaw app` → then configure the long connection in the Feishu console. If errors persist, try stopping the xclaw service and restarting `xclaw app`.
...
Find `channels.feishu` in your agent's `agent.json` (e.g., `~/.xclaw/workspaces/default/agent.json`). Only **App ID** and **App Secret** are required (copy from the Feishu console under Credentials & basic info):
...
> **Note:** You can also fill in **App ID** and **App Secret** in the Console UI, but you must restart the xclaw service before continuing with the long-connection configuration.
...
3. For xClaw to read iMessage data, **Terminal** (or the app you use to run `xclaw app`) and **Messages** need **Full Disk Access** (System Settings → Privacy & Security → Full Disk Access).
...
4. Set the iMessage database path. The default is `~/Library/Messages/chat.db`; use this unless you've moved the database. You can configure it in either of these ways:
   ...
   - Or edit the agent workspace `agent.json` (usually at `~/.xclaw/workspaces/default/agent.json`):
...
Find `channels.discord` in your agent's `agent.json` (e.g., `~/.xclaw/workspaces/default/agent.json`) and fill in the fields, for example:
...
5. In **Developer settings**, get **AppID** and **AppSecret** (ClientSecret) and fill them into config (see below). Add your server’s **IP to the whitelist** — only whitelisted IPs can call the Open API outside sandbox.
   ...
   > **Tip:** If you are using ModelScope Creative Space to deploy xClaw, the IP whitelist for QQ channel should be: `47.92.200.108`
...
In your agent's `agent.json` (e.g., `~/.xclaw/workspaces/default/agent.json`), find `channels.qq` and set `app_id` and `client_secret` to the values above:
...
The **OneBot** channel connects xClaw to [NapCat](https://github.com/NapNeko/NapCatQQ), [go-cqhttp](https://github.com/Mrs4s/go-cqhttp), [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), or any other [OneBot v11](https://github.com/botuniverse/onebot-11) compatible implementation via **reverse WebSocket**.
...
xClaw starts a WebSocket server; the OneBot implementation (e.g. NapCat) connects to it as a client:

```
NapCat  ──reverse WS──▶  xClaw (:6199/ws)
```
...
3. Go to **Network Config** → **New** → **WebSocket Client** (reverse WS):
   ...
   - Access Token: same as `access_token` in xClaw config (optional)
...
> **Docker Compose tip:** When running xClaw and NapCat in Docker Compose, set the NapCat reverse WS URL to `ws://xclaw:6199/ws` (using the service name).
...
> **Note:** Audio and video are received at the channel level, but require xClaw's transcription provider (`transcription_provider_type`) to be configured for the LLM to process them. Without transcription, voice messages are shown as placeholders.
...
**Method 2:** Fill in `agent.json` (e.g., `~/.xclaw/workspaces/default/agent.json`)
...
- **Authentication**: On first use, scan a QR code to authorize. The token is automatically persisted to a local file (default `~/.xclaw/weixin_bot_token`), so you won't need to scan again on subsequent starts.
...
1. Open the xClaw Web Console and go to **Settings → Channels → WeChat Personal (iLink)**.
...
You can also configure directly in the agent workspace `agent.json` (e.g., `~/.xclaw/workspaces/default/agent.json`):

```json
"weixin": {
  "enabled": true,
  "bot_token": "your_bot_token",
  "bot_token_file": "~/.xclaw/weixin_bot_token",
  "base_url": "",
  "media_dir": "~/.xclaw/media",
  "dm_policy": "open",
  "group_policy": "open"
...
WEIXIN_BOT_TOKEN_FILE=~/.xclaw/weixin_bot_token
WEIXIN_MEDIA_DIR=~/.xclaw/media
...
Find `channels.telegram` in your agent's `agent.json` (e.g., `~/.xclaw/workspaces/default/agent.json`) and fill in the fields, for example:
...
2. Fuzzy match subscription and automatic push
   ...
   Subscribe to the wildcard topic `/server/+/up`. Messages will be automatically pushed to the corresponding topic based on the client's `client_id`. For example, after a client pushes a message to `/server/client_a/up`, xClaw will push the message to `/client/client_b/down` after processing.
...
3. Redirected topic push
   ...
   Messages will be pushed to `client/client_b/down` based on the `redirect_client_id` attribute, enabling cross-topic push. In IoT scenarios, with xClaw as the core, autonomous message pushing between multiple devices can be achieved according to individual requirements.

---

## Matrix

The Matrix channel connects xClaw to any Matrix homeserver using the [matrix-nio](https://github.com/poljar/matrix-nio) library. It supports text messaging in both direct messages and group rooms.
...
Find `channels.matrix` in your agent's `agent.json` (e.g., `~/.xclaw/workspaces/default/agent.json`):
...
Save the file; the channel will reload automatically if xClaw is already running.
...
The XiaoYi channel connects xClaw via **A2A (Agent-to-Agent) protocol** over WebSocket to Huawei's AI assistant platform.
...
The Voice channel enables phone call interactions with xClaw via Twilio ConversationRelay, supporting Speech-to-Text (STT) and Text-to-Speech (TTS) for voice-based conversations.

### Prerequisites

1. **Twilio Account**: Register at [Twilio](https://www.twilio.com/) and obtain credentials
2. **Cloudflare Tunnel** (or similar): Expose your local xClaw service to the public internet for Twilio webhook callbacks
...
Twilio needs to reach xClaw's webhook endpoint via the public internet, so you need to expose your local service.
...
```json
{
  "channels": {
    "voice": {
      ...
      "language": "en-US",
      "welcome_greeting": "Hi! This is xClaw. How can I help you?"
    }
  }
}
```
...
After configuration, simply call your Twilio phone number to have a voice conversation with xClaw:

1. Dial the phone number
2. After hearing the welcome greeting, start speaking
3. xClaw converts speech to text and processes it through the Agent
...
from xclaw.app.channels.base import BaseChannel
from xclaw.app.channels.schema import ChannelType
...
- **Directory**: Channels under the working dir at `custom_channels/` (default `~/.xclaw/custom_channels/`) are loaded at runtime. The manager scans `.py` files and packages (subdirs with `__init__.py`), loads `BaseChannel` subclasses, and registers them by the class’s `channel` attribute.
- **Install**: `xclaw channels install <key>` creates a template `<key>.py` in `custom_channels/` for you to edit, or use `--path <local path>` / `--url <URL>` to copy a channel module from disk or the web. `xclaw channels add <key>` does the same and also adds a default entry to config (with optional `--path`/`--url`).
- **Remove**: `xclaw channels remove <key>` deletes that channel’s module from `custom_channels/` (custom channels only; built-ins cannot be removed). By default it also removes the key from `channels` in `config.json`; use `--keep-config` to leave config unchanged.
- **Config**: `ChannelConfig` uses `extra="allow"`, so any channel key can appear under `channels` in `config.json`. Use `xclaw channels config` for interactive setup or edit config by hand.

### HTTP route registration

For channels that require webhook callbacks (e.g., WeChat, Slack, LINE), you can register custom HTTP routes by exporting a `register_app_routes` callable in your module — no changes to xClaw's core source required.

At startup, xClaw scans modules in `custom_channels/` for a `register_app_routes` export. If found, it is called with the FastAPI `app` instance, allowing the channel to register any routes it needs.
...
def register_app_routes(app):
    ...
    async def echo_callback(request):
        ...
        from xclaw.app.channels.base import TextContent
...
**Real-world example**: WeChat ClawBot integration ([PR #2140](https://github.com/agentscope-ai/xClaw/pull/2140), [Issue #2043](https://github.com/agentscope-ai/xClaw/issues/2043)) uses this mechanism to register the `/api/wechat/callback` route with Tencent's official SDK for message delivery.
