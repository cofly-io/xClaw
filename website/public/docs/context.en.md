...
xClaw context management uses two parallel offload paths to handle the limited context window:
...
xClaw's context consists of two components:
...
When the context approaches its limit, xClaw automatically triggers compaction, condensing old conversations into a structured summary.
...
Configuration is located in `~/.xclaw/config.json` under `agents.running`:
