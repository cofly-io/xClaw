...
本页说明 xClaw 是什么、能做什么、以及如何按文档一步步上手。

---

## xClaw 是什么？

xClaw 是一款**个人助理型产品**，部署在你自己的环境中。
...
xClaw 由 [AgentScope 团队](https://github.com/agentscope-ai) 基于
...
1. **在聊天软件里对话**
   在钉钉、飞书、微信、Discord、Telegram等app里发消息，xClaw 在同一 app 内回复，
   查资料、记待办、回答问题等都由当前启用的 Skills 完成。一个 xClaw 可同时接入多个
   app，你在哪个频道聊，它就在哪个频道回。

2. **定时自动执行**
   无需每次手动发消息，xClaw 可按你设定的时间自动运行：
   - 定时向某频道发送固定文案（如每天 9 点发「早上好」）；
   - 定时向 xClaw 提问并将回答发到指定频道（如每 2 小时问「我有什么待办」并发到钉钉）；
   - 定时执行「自检/摘要」：用你写好的一串问题问 xClaw，把回答发到你上次对话的频道。

装好、接好至少一个频道并启动服务后，你就可以在钉钉、飞书、QQ 等里与 xClaw 对话，并享受定时
...
- **控制台** — xClaw 内置的 Web 管理界面，可以在控制台中对话、配置频道、管理技能、
  设置模型等。详见 [控制台](./console)。
- **频道** — 你和 xClaw 对话的「场所」（钉钉、飞书、QQ、Discord、iMessage 等）。在
  [频道配置](./channels) 中按步骤配置。
- **心跳** — 按固定间隔用你写好的一段问题去问 xClaw，并可选择把回答发到你上次使用的
  频道。详见 [心跳](./heartbeat)。
- **定时任务** — 多条、各自独立配置时间的任务（每天几点发什么、每隔多久问 xClaw 什么等），
  通过控制台或 [CLI](./cli) 管理。
- **技能池与工作区技能** — 技能池是共享的技能仓库，工作区技能是某个智能体真正运
  行时使用的技能副本。详见 [Skills](./skills)。
- **MCP 和工具** — MCP（Model Context Protocol）是一种标准协议，允许接入外部
  工具服务器扩展能力。工具是 xClaw 内置的基础能力（如读写文件、执行命令、
  浏览器等）。详见 [MCP和工具](./mcp) 。
- **智能体/工作区** — 从 v0.1.0 开始，xClaw 支持多智能体，允许运行多个独立的
  AI 智能体。每个智能体拥有独立的工作区、配置、记忆、技能和对话历史，智能体之间
  还可以通过协作技能互相通信、共同完成复杂任务。详见 [多智能体](./multi-agent)。
- **安全机制** — xClaw 提供多层安全防护，包括工具防护（拦截危险命令参数）、
...
2. **[控制台](./console)** — 服务启动后，在浏览器中打开控制台（`http://127.0.0.1:8088/`），
   **这是配置与使用 xClaw 的中心枢纽**。先在控制台体验对话、配置模型，有助于理解
   xClaw 的工作方式。
3. **[模型](./models)** — 配置云端 LLM 提供商的 API Key，或下载本地模型。这是使用
   xClaw 的**必要前提**。
4. **按需配置与使用**：
   - [频道配置](./channels) — 接入钉钉 / 飞书 / 微信 / Discord / Telegram 等，在对应 app 里与 xClaw 对话；
   - [Skills](./skills) — 了解与扩展 xClaw 能力；
