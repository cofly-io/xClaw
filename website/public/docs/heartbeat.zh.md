...
「心跳」在 xClaw 里指的是：**按固定间隔，用你写好的一段「问题」去问 xClaw，并可选择把 xClaw 的回复发到你上次对话的频道**。适合做「定期自检、每日摘要、定时提醒」——不用你主动发消息，xClaw 到点就干活。
...
1. 在当前智能体的 workspace 里有一个**心跳查询文件**（默认文件名为 **HEARTBEAT.md**，可用环境变量 `QWENPAW_HEARTBEAT_FILE` 改名）。里面写的是**每次心跳要问 xClaw 的内容**（一段或几段话都行，xClaw 会当成一条用户消息）。
2. 当配置里 **`enabled` 为 true** 时，系统按你配置的 **every**（间隔字符串或五段 Cron）执行一次：读取该文件 → 用这段内容去问 xClaw → xClaw 回复。
3. **发不发到频道** 由配置里的 **target** 决定：
   - **main**：只跑 xClaw，不把回复发到任何频道（适合只做「自检」、结果自己看日志或别处）。
   - **last**：把 xClaw 的回复发到你**上次和 xClaw 对话的那个频道/会话**（例如上次你在钉钉和它聊，这次心跳的回复就发到钉钉）。
...
`<QWENPAW_WORKING_DIR>` 默认是 `~/.xclaw`，也可用环境变量 `QWENPAW_WORKING_DIR` 覆盖；`<agent_id>` 与当前智能体一致（例如 `default`）。

文件名默认 `HEARTBEAT.md`，可通过 **`QWENPAW_HEARTBEAT_FILE`** 改成别的名字；路径始终是「该智能体 workspace 根目录 + 该文件名」。

内容就是「每次要问 xClaw 什么」，纯文本或 Markdown 都行，xClaw 会整体当作一条用户消息。
...
初始化时如果执行过 `xclaw init`（没加 `--defaults`），会提示你是否编辑 HEARTBEAT.md；选是会用系统默认编辑器打开。你也可以之后随时用任何编辑器改这个文件，保存即可，下次心跳会用到新内容。
...
示例（开启心跳，只跑 xClaw、不发到频道，每 30 分钟）——写在对应智能体的 **`agent.json`** 中：
...
> 需要「每天 9 点发早安」「每 2 小时问待办并发到钉钉」这类多条任务？用 [CLI](./cli) 的 `xclaw cron create` 做定时任务，不用心跳。
