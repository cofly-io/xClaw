...
This page explains what xClaw is, what it can do, and how to get started
...
xClaw is a **personal assistant** that runs in your own environment.
...
xClaw is built by the [AgentScope team](https://github.com/agentscope-ai) on
...
## How do you use xClaw?

There are two main ways to use xClaw:

1. **Chat in messaging apps**
   Send messages in DingTalk, Feishu, WeChat, Discord, Telegram, or other apps,
   and xClaw replies in the same app. Whether looking things up, managing todos,
   or answering questions — it all depends on your enabled Skills. A single
   xClaw instance can connect to multiple apps, responding in whichever channel
   you're using.

2. **Scheduled execution**
   No need to send messages manually each time. xClaw can run automatically at
   set times:
   - Send a fixed message to a channel (e.g., "Good morning" to DingTalk at 9am);
   - Ask xClaw a question and send the answer to a channel (e.g., every 2 hours
     ask "What are my todos?" and post to DingTalk);
   - Run a scheduled check-in or digest: use your prepared questions to query
     xClaw and send the answers to your last active channel.

After installation, connecting at least one channel, and starting the service,
you can chat with xClaw in DingTalk, Feishu, QQ, and other apps, and enjoy
...
- **Console** — xClaw's built-in web management interface. Chat, configure
  channels, manage skills, set up models, and more. See [Console](./console).
- **Channel** — Where you talk to xClaw (DingTalk, Feishu, QQ, Discord,
  iMessage, etc.). Configure step by step in [Channels](./channels).
- **Heartbeat** — Ask xClaw your prepared questions at fixed intervals, and
...
- **Scheduled tasks** — Multiple tasks, each with independent time configuration
  (send X daily at 9am, ask xClaw Y every 2 hours, etc.), managed through the
  Console or [CLI](./cli).
...
- **MCP and tools** — MCP (Model Context Protocol) is a standard protocol for
  connecting external tool servers to extend capabilities. Tools are xClaw's
  built-in core abilities (reading/writing files, executing commands, browser,
  etc.). See [MCP and Tools](./mcp).
- **Agent/Workspace** — Starting from v0.1.0, xClaw supports multi-agent,
...
- **Security mechanisms** — xClaw provides multi-layer security, including
...
2. **[Console](./console)** — After the service starts, open the Console in
   ...
   configuring and using xClaw.** Start by chatting and configuring models in
   the Console to understand how xClaw works.
3. **[Models](./models)** — Configure API keys for cloud LLM providers or
   download local models. This is a **required prerequisite** for using xClaw.
4. **Configure and use as needed**:
   - [Channels](./channels) — Connect DingTalk / Feishu / WeChat / Discord / Telegram to chat with xClaw in those apps;
   - [Skills](./skills) — Understand and extend xClaw's capabilities;
