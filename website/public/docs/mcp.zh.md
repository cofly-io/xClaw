...
xClaw 通过 **MCP（模型上下文协议）** 连接外部服务，并提供一组 **内置工具**，让智能体能够访问文件系统、执行命令、浏览网页等。
...
1. **内置工具**：开箱即用，由 xClaw 核心提供，如文件操作、命令执行、浏览器自动化等
...
**MCP（模型上下文协议，Model Context Protocol）** 允许 xClaw 连接到外部 MCP 服务器，扩展智能体访问文件系统、数据库、API 等外部资源的能力。
...
xClaw 提供了一组开箱即用的内置工具，智能体可以直接调用这些工具完成各种任务。
