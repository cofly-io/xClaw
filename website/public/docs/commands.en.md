...
- 🖼️ - Supports image input
- 🎥 - Supports video input
- _(user-added)_ - User-added model (via `xclaw models add-model` command)
...
Commands for controlling and monitoring xClaw's runtime status. These commands execute directly without going through the Agent.
...
```
/status                    # In chat
xclaw daemon status        # From terminal
```
...
```
/restart                   # In chat
xclaw daemon restart       # From terminal (prints instructions only)
```
...
xclaw daemon reload-config      # From terminal
```
...
Display xClaw version number, working directory path, and log file path.

**Usage:**

```
/daemon version            # In chat
xclaw daemon version       # From terminal
```
...
View the last N lines of `xclaw.log` in the working directory. Default 100 lines, maximum 2000 lines.
...
xclaw daemon logs -n 200   # From terminal, specify 200 lines
```
...
```bash
xclaw daemon status --agent-id abc123
xclaw daemon version --agent-id abc123
```
