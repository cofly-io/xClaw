# xClaw Desktop Application Guide
...
This guide explains how to install and use the xClaw Desktop application on Windows and macOS.

[releases]: https://github.com/agentscope-ai/xClaw/releases
...
1. **Download the installer**
   Download `xClaw-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\xClaw`
...
#### **xClaw Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "xClaw Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **xClaw Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "xClaw Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "xClaw Desktop (Debug)" mode to view terminal output for error messages
...
1. **Download the archive**
   Download `xClaw-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `xClaw.app`

3. **Move to Applications folder (Optional)**
   Drag `xClaw.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

xClaw is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `xClaw.app`
...
2. Scroll down to find a message like:
   _"'xClaw' was blocked from use because it cannot verify the developer"_
...
- Double-click `xClaw.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.xclaw/desktop.log`
...
cd /Applications  # or wherever your xClaw.app is located

# Set environment variables and launch
APP_ENV="$(pwd)/xClaw.app/Contents/Resources/env"
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/xClaw/issues)
- **Packaging documentation**: `scripts/pack/README.md` - Technical details and local build guide
- **Log locations**:
  - Windows: View in Debug mode terminal, or `%USERPROFILE%\.xclaw\` directory
  - macOS: `~/.xclaw/desktop.log`
