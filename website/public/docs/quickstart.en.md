...
This section describes multiple ways to install xClaw:
...
on Windows). This installs the `xclaw` command.
...
To overwrite existing config, use `xclaw init --force` (you will be prompted).
...
channels, xClaw will reply there. Otherwise, you can complete this section
...
Then open a new terminal (the installer adds xClaw to your PATH automatically).
...
>      - Check if `uv` is available: Enter `uv --version` in CMD. If a version number appears, **only configure the xClaw path**. If you receive `'uv' is not recognized as an internal or external command, operable program or batch file,` configure both paths.
...
>      - xClaw path: Typically `%USERPROFILE%\.xclaw\bin`.
...
To upgrade, simply re-run the install command. To uninstall, run `xclaw uninstall`.
...
Images are on **Docker Hub** (`agentscope/xclaw`). Image tags: `latest` (stable);
`pre` (PyPI pre-release). Also available on Alibaba Cloud ACR for users in China:
`agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/xclaw` (same tags).
...
To deploy xClaw on Alibaba Cloud, use the ECS one-click deployment:

1. Open the [xClaw Alibaba Cloud ECS deployment link](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) and fill in the deployment parameters as prompted;
...
For detailed steps and instructions, see [Alibaba Cloud Developer Community: Deploy your AI assistant with xClaw in 3 minutes](https://developer.aliyun.com/article/1713682).

---

## Option 5: ModelScope Studio one-click setup (no installation)

If you don't want to install Python locally, you can deploy xClaw to the cloud
through ModelScope Studio:

1. First go to [ModelScope](https://modelscope.cn/register?back=%2Fhome) to register and log in;
2. Open [xClaw Studio](https://modelscope.cn/studios/fork?target=AgentScope/xClaw) and complete the one-click setup.

**Important**: Set your Studio to **non-public**, or others may control your xClaw.
...
xClaw's desktop application without manually configuring Python environments
or running commands.
...
1. **Download the installer**
   Go to [GitHub Releases](https://github.com/agentscope-ai/xClaw/releases) to download the version for your system:

   - Windows: `xClaw-Setup-<version>.exe`
   - macOS: `xClaw-<version>-macOS.zip`

2. **Install and launch**

   - **Windows**: Double-click the `.exe` file to install following the wizard, then double-click the desktop shortcut to launch
   - **macOS**: Extract the `.zip` to get `xClaw.app`, first time requires right-click and select "Open" to bypass system security restrictions
...
xClaw needs a large language model to work. You can choose either option:
...
- xClaw Local (llama.cpp): download `llama.cpp` inside xClaw Local provider settings, see [Models - Local providers Configuration](./models) for details.
...
- For xClaw Local (llama.cpp), you can download models directly from the provider settings in the Console, or manually place GGUF model files in the local models directory (default `~/.xclaw/local_models/models/<org>/<model>`, for example: `~/.xclaw/local_models/models/Qwen/Qwen3-0.6B-GGUF`).
- For Ollama and LM Studio, you need to add models in their respective services first, then xClaw can automatically fetch the model list and connect to them.
...
page to test functionality and confirm xClaw can reply normally.
...
Chat with xClaw in DingTalk, Feishu, QQ, Discord, iMessage, and other apps:
...
4. After saving, you can send messages to xClaw in the corresponding app

#### 🔧 Enable and extend skills

Give xClaw more capabilities (PDF processing, Office documents, news summaries, etc.):
...
Let xClaw execute tasks automatically:

- **Scheduled tasks**: Create in Console **Control → Scheduled tasks**, or use `xclaw cron` command in [CLI](./cli)
