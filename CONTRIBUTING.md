...
- **Check [Open Issues](https://github.com/agentscope-ai/xClaw/issues)** and any [Projects](https://github.com/agentscope-ai/xClaw/projects) or roadmap labels.
...
xClaw is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

xClaw supports multiple providers, including cloud providers (such as DashScope, ModelScope) and local providers (such as Ollama, LM Studio). We also welcome new model providers to enrich user choices.
...
If the above conditions are met, you can create a new Provider instance in `src/xclaw/providers/provider_manager.py` and register it in the `ProviderManager` class to make it a built-in provider in xClaw.
...
Channels are how xClaw talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so xClaw can work with your favorite IM or bot platform.
...
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/xclaw/app/channels/base.py`):
...
- **Discovery:** Built-in channels are registered in `src/xclaw/app/channels/registry.py`. **Custom channels** are loaded from the working directory: place a module (e.g. `custom_channels/telegram.py` or a package `custom_channels/telegram/`) that defines a `BaseChannel` subclass with a `channel` attribute.
- **CLI:** Users install/add channels with:
  - `xclaw channels install <key>` — create a template or copy from `--path` / `--url`
  - `xclaw channels add <key>` — install and add to config
  - `xclaw channels remove <key>` — remove custom channel from `custom_channels/`
  - `xclaw channels config` — interactive config
...
**Skills** define what xClaw can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/xclaw/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
- **Skills Hub:** xClaw supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hub’s packaging format.
...
xClaw aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **Install and run:** One-line install (`install.sh`), `pip` install, and `xclaw init` / `xclaw app` should work (or be clearly documented) on each supported platform. Fixes to install or startup on a given OS are valuable.
...
- **MCP (Model Context Protocol):** xClaw supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
- **Documentation:** Fixes and improvements to [the docs](https://xclaw.agentscope.io/) (under `website/public/docs/`) and README are always welcome.
...
- **Discussions:** [GitHub Discussions](https://github.com/agentscope-ai/xClaw/discussions)
- **Bugs and features:** [GitHub Issues](https://github.com/agentscope-ai/xClaw/issues)
- **Community:** DingTalk group (see [README](README.md)) and [Discord](https://discord.gg/eYMpfnkG8h)

Thank you for contributing to xClaw. Your work helps make it a better assistant for everyone. 🐾
