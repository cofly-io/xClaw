...
def _detect_install_method() -> str:
    """Detect how xClaw was installed based on environment signals."""
...
def get_system_info() -> dict[str, Any]:
    ...
    - xclaw_version: xClaw version string
    - install_method: How xClaw was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running xclaw (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as app_ver

    ver = _safe_get(lambda: app_ver, "unknown")
    info = {
...
def _get_current_version() -> str:
    """Get the current xClaw version string."""
    try:
        from ..__version__ import __version__ as app_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when xClaw is upgraded (or downgraded) to a
    version that hasn't been collected before.

    Args:
        working_dir: Path to xClaw working directory
    ...
    try:
        ...
        # v1.1 compat: single-version field (legacy key + current)
        return marker_data.get("xclaw_version", "") == current or (
            marker_data.get("qwenpaw_version", "") == current
        )
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("xclaw_version", "") or old_data.get(
                        "qwenpaw_version",
                        "",
                    )
                    if old_ver:
