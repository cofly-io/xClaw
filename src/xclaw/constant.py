...
if _legacy_copaw_dir.exists():
    ...
    WORKING_DIR = (
        Path(_get_env("QWENPAW_WORKING_DIR", "~/.xclaw"))
        .expanduser()
        .resolve()
    )
SECRET_DIR = (
...
def _resolve_docs_dir() -> Path | None:
    """Find xClaw documentation directory across all install methods."""
...
# Builtin Q&A helper profile.  agent_id keeps "xClaw" prefix for existing
...
# Custom channel modules (installed via `xclaw channels install`); manager
# loads BaseChannel subclasses from here.
CUSTOM_CHANNELS_DIR = WORKING_DIR / "custom_channels"

# Plugin directory (installed via `xclaw plugin install`)
