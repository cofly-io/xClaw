...
def get_xclaw_version() -> str:
    """Return the installed xClaw package version, or ``'unknown'``."""
    try:
...
def generate_backup_id() -> str:
    ...
    Format: ``xclaw-{version}-{YYYYMMDDTHHmmssZ}-{short8}``

    Example: ``xclaw-1.2.3-20260420T093000Z-ab12cd34``
    """
    ver = re.sub(r"[^a-zA-Z0-9._-]", "_", get_xclaw_version())
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    short = uuid4().hex[:8]
    return f"xclaw-{ver}-{ts}-{short}"
...
def finalize_backup_meta(meta, agent_count: int) -> None:
    ...
    meta.qwenpaw_version = get_xclaw_version()
