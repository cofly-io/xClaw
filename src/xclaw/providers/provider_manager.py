...
class ProviderManager:  # pylint: disable=too-many-public-methods
    ...
    def _normalize_provider_id(provider_id: str) -> str:
        ...
        Maps legacy ``copaw-local`` / ``qwenpaw-local`` to ``xclaw-local``.
        """
        if provider_id in ("copaw-local", "qwenpaw-local"):
            return "xclaw-local"
        return provider_id
    ...
    def _migrate_copaw_config(self) -> None:
        """Migrate legacy local provider configs to xclaw-local."""
        # 1. Migrate active model configuration (only provider_id)
        if self.active_model and self.active_model.provider_id in (
            "copaw-local",
            "qwenpaw-local",
        ):
            self.active_model.provider_id = "xclaw-local"
            self.save_active_model(self.active_model)
            logger.info(
                "Migrated active model provider id to 'xclaw-local'",
            )

        # 2. Migrate stored builtin provider JSON (copaw- or qwenpaw-era filenames)
        for legacy_name in ("copaw-local.json", "qwenpaw-local.json"):
            legacy_path = self.builtin_path / legacy_name
            if not legacy_path.exists():
                continue
            try:
                with open(legacy_path, "r", encoding="utf-8") as f:
                    old_config = json.load(f)
                provider = self.builtin_providers.get("xclaw-local")
                if not provider:
                    return
                ...
                legacy_path.unlink()
                logger.info(
                    "Migrated provider config from '%s' to xclaw-local storage",
                    legacy_name,
                )
            except Exception as exc:
                logger.warning("Failed to migrate %s: %s", legacy_name, exc)
    ...
    def _migrate_legacy_providers(self):
        ...
        if legacy_path.exists() and legacy_path.is_file():
            ...
            if active_model:
                try:
                    # Convert legacy local provider ids
                    if active_model.get("provider_id") in (
                        "copaw-local",
                        "qwenpaw-local",
                    ):
                        active_model["provider_id"] = "xclaw-local"
    ...
    def _init_from_storage(self):
        ...
        # Migrate copaw-local to xclaw-local for backwards compatibility
    ...
    async def _resume_local_model(self, local_manager) -> None:
        ...
        local_models = self.get_provider("xclaw-local").extra_models
