...
# Template for `xclaw channels install <key>` stub (channel key substituted).
...
def configure_voice(
    ...
    welcome_greeting = click.prompt(
        ...
        or "Hi! This is xClaw. How can I help you?",
        type=str,
    )
...
@click.option(
    "--target-user",
    required=True,
    help=("Target user ID (REQUIRED, get from 'xclaw chats list' query)"),
)
@click.option(
    "--target-session",
    required=True,
    help=("Target session ID (REQUIRED, get from 'xclaw chats list' query)"),
)
@click.option(
    "--text",
    required=True,
...
def send_cmd(
    ...
    Complete Usage Flow:
      Step 1 - Query available sessions (REQUIRED):
        xclaw chats list --agent-id my_bot --channel console
      ...
      Step 3 - Send message using queried parameters:
        xclaw channels send --agent-id my_bot --channel console \\
    ...
    Examples with jq automation:
      ...
      SESSIONS=$(xclaw chats list --agent-id bot --channel console)
      USER=$(echo "$SESSIONS" | jq -r '.[0].user_id')
      SESSION=$(echo "$SESSIONS" | jq -r '.[0].session_id')

      # Send message
      xclaw channels send --agent-id bot --channel console \\
        --target-user "$USER" --target-session "$SESSION" \\
        --text "Automated notification"

    \b
    Prerequisites:
      1. MUST use 'xclaw chats list' to get valid target-user and
         target-session
