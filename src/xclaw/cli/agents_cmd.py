...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  xclaw agents chat --background --task-id {task_id}")
    ...
    try:
        ...
        if status == "finished":
            ...
            if task_status == "completed":
                ...
                click.echo(
                    f"  xclaw agents chat --background --task-id {task_id}",
                )
...
def agents_group() -> None:
    ...
    Examples:
      xclaw agents list
      xclaw agents chat --from-agent bot_a --to-agent bot_b --text "..."
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      xclaw agents list
      xclaw agents list --base-url http://192.168.1.100:8088
...
def chat_cmd(
    ...
    \b
    Prerequisites:
      1. Use 'xclaw agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'xclaw chats list' to find existing sessions (optional)
