...
def _get_api_base() -> Optional[str]:
    """Return the base URL of the running xClaw API, or None.
...
def _api_install_plugin(source: str, force: bool = False) -> bool:
    """Send a hot-install request to the running xClaw API.
...
def _api_upload_plugin(zip_path: Path, force: bool = False) -> bool:
    """Send a ZIP file to the running xClaw API for hot-install.
...
def _api_uninstall_plugin(plugin_id: str) -> bool:
    """Send a hot-uninstall request to the running xClaw API.
...
def _is_running() -> bool:
    """Return whether xClaw is currently running.

    Returns:
        ``True`` if the API is reachable, ``False`` otherwise.
    """
    from ..config.utils import is_xclaw_running

    return is_xclaw_running()
...
def install(source: str, force: bool):
    ...
    When xClaw is running, the plugin is hot-loaded immediately via
    the API (no restart required).  When xClaw is stopped, the
    plugin files are copied and will be loaded on next start.

    Examples:
        xclaw plugin install examples/plugins/idealab-provider
    ...
    if _is_running():
        click.echo(
            "xClaw is running — using hot-install via API...",
        )
    ...
    click.echo("\nNext steps:")
    click.echo("   1. Start xClaw to load the plugin")
...
def uninstall(plugin_id: str):
    ...
    When xClaw is running, the plugin is unloaded immediately via
    the API (no restart required).  When xClaw is stopped, only the
    ...
    if _is_running():
        click.echo(
            "xClaw is running — using hot-uninstall via API...",
        )
