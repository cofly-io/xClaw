...
def _check_xclaw_not_running():
    """Check if xClaw is not running, exit if it is."""
    from ..config.utils import is_xclaw_running

    if is_xclaw_running():
        click.echo(
            "❌ xClaw is currently running. Please stop it first:",
            err=True,
        )
        click.echo("   xclaw shutdown", err=True)
        click.echo(
            "\n💡 Plugin operations are only allowed when xClaw is stopped.",
        )
        raise click.Abort()
...
def install(source: str, force: bool):
    ...
    # Check if xClaw is running
    _check_xclaw_not_running()
    ...
    click.echo("\n💡 Next steps:")
    click.echo("   1. Restart xClaw to load the plugin")
...
def uninstall(plugin_id: str):
    ...
    try:
        ...
        click.echo("💡 Restart xClaw to apply changes")
    except Exception as e:
