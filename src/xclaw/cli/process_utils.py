...
def _matches_xclaw_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a xClaw CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m xclaw {subcommand}",
            f" xclaw {subcommand}",
            f" -m qwenpaw {subcommand}",
            f" qwenpaw {subcommand}",
            f"__main__.py {subcommand}",
            f'xclaw.exe" {subcommand}',
            f"xclaw.exe {subcommand}",
...
def _is_xclaw_service_command(command: str) -> bool:
    """Return whether the command line looks like a local xClaw app."""
    return _matches_xclaw_cli_command(command, "app")


def _is_xclaw_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a xClaw CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name in ("xclaw", "qwenpaw") or _matches_xclaw_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local xClaw service."""
    if not host:
        return []
