...
def chats_group() -> None:
    ...
    \b
    Common examples:
      xclaw chats list                    # List all chats
      xclaw chats list --user-id alice    # Filter by user
      xclaw chats get <chat_id>           # View details
      xclaw chats create --session-id s1 --user-id u1
      xclaw chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      xclaw chats list
      xclaw chats list --user-id alice
      xclaw chats list --channel discord
      xclaw chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `xclaw chats list`.
...
def create_chat(
    ...
    Inline creation examples:
      xclaw chats create --session-id "discord:alice" \\
    ...
    JSON file creation example:
      xclaw chats create -f chat.json
    """
    base_url = resolve_base_url(ctx, base_url)
...
def update_chat(
    ...
    \b
    Examples:
      xclaw chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
    payload = {"name": name}
...
def delete_chat(
    ...
    \b
    Examples:
      xclaw chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
