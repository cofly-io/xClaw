    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'xclaw[local]'",
                fg="red",
            ),
        )
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'xclaw models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'xclaw models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'xclaw models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'xclaw models add-model' to add models, "
        "then 'xclaw models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      xclaw models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
    ...
    click.echo(
        ...
        "  xclaw models set-llm  (select 'xclaw-local' provider)",
    )
...
def list_local_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'xclaw models download <repo_id>' to download one.")
        return
