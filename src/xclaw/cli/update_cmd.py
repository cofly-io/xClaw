...
from .process_utils import (
    ...
    _is_xclaw_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/xclaw/json"
...
class InstallInfo:
    """Information about the current xClaw installation."""
...
class RunningServiceInfo:
    """Detected xClaw service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published xClaw version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest xClaw version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest xClaw version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest xClaw version.",
        )
    return version
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("xclaw")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running xClaw HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local xClaw service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_xclaw_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a xClaw HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running xClaw service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"xClaw service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `xclaw shutdown` should be used before updating."""
    click.echo("")
    ...
    click.secho(
        "Running `xclaw shutdown` will forcibly terminate the current "
        "xClaw backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `xclaw shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `xclaw shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "xclaw"]
    ...
    click.echo("")
    click.echo("Running `xclaw shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `xclaw shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`xclaw shutdown` failed. Please stop the running xClaw "
            "service manually before running `xclaw update`.",
        )
...
def _build_upgrade_command(
    ...
    package_spec = f"xclaw=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from xclaw.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[xclaw] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[xclaw] Updating xClaw "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[xclaw] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[xclaw] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[xclaw] Update completed successfully.")
        click.echo(
            "[xclaw] Please restart any running xClaw service "
            "to use the new version.",
        )
    else:
        click.echo(f"[xclaw] Update failed with exit code {return_code}.")
        click.echo(
            "[xclaw] Please fix the error above and run "
            "`xclaw update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade xClaw in the current Python environment."""
    ...
    if version_check is False:
        click.echo("xClaw is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `xclaw update`, or rerun "
                "without `--yes` to confirm a forced `xclaw shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `xclaw shutdown`. "
                "Please stop it manually before running `xclaw update`.",
            )

    if not yes and not click.confirm(
        f"Update xClaw to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
