# -*- coding: utf-8 -*-
"""xclaw uninstall — remove the xClaw environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove xClaw PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# xClaw" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# xClaw\nexport PATH=\"\$HOME/\.xclaw/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove xClaw environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL xClaw data in {wd}")
    else:
        click.echo(
            "This will remove the xClaw Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("xClaw uninstalled. Please restart your terminal.")
