...
class WeixinChannel(BaseChannel):
    ...
    async def _on_message(
        ...
        try:
            ...
            for item in item_list:
                ...
                # When a user replies to a message in WeChat, the item contains a
                # ``ref_msg`` field whose ``message_item`` mirrors the structure
                # of a normal item (type 1-5).
    ...
    async def _process_quoted_ref_msg(
        ...
        Extracts text and media from the referenced message and prepends quoted
        text to *text_parts* / appends media to *content_parts*.
        """
        ...
        if quoted_type == 1:
            ...
            if encrypt_query_param:
                ...
                if path:
                    content_parts.append(
                        ImageContent(type=ContentType.IMAGE, image_url=path),
                    )
                else:
            ...
            if encrypt_query_param:
                ...
                if path:
                    content_parts.append(
                        FileContent(type=ContentType.FILE, file_url=path),
                    )
                else:
            ...
            if encrypt_query_param:
                ...
                if path:
                    content_parts.append(
                        VideoContent(type=ContentType.VIDEO, video_url=path),
                    )
                else:
    ...
    async def _send_text_direct(
        ...
        if isinstance(resp, dict):
            ...
            if ret != 0 or errcode != 0:
                logger.warning(
                    "weixin send_text rejected: ret=%s errcode=%s user=%s",
                    ret,
                    errcode,
                    to_user_id,
                )
    ...
    async def _send_media_file(
        ...
        try:
            ...
            if isinstance(resp, dict):
                ...
                if ret != 0 or errcode != 0:
                    logger.warning(
                        "weixin send_media rejected: "
                        "ret=%s errcode=%s user=%s type=%s",
                        ret,
                        errcode,
                        to_user_id,
                        content_type,
    ...
    async def _send_content_parts_immediate(
        ...
        for p in parts:
            ...
            if t == ContentType.TEXT and text_val:
                ...
                if audio_url and not str(audio_url).startswith("data:"):
                    await self._send_media_file(
                        to_user_id,
                        context_token,
                        str(audio_url),
