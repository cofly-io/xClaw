# -*- coding: utf-8 -*-
"""
MatrixChannel: xClaw BaseChannel implementation for Matrix (via matrix-nio).
...
logger = logging.getLogger("xclaw.channels.matrix")

# ---------------------------------------------------------------------------
# Lazy import of xClaw base types so this file can be syntax-checked without
# xclaw installed (it's only executed inside a xclaw environment).
# ---------------------------------------------------------------------------
try:
...
# Known xClaw slash commands — used to decide whether to strip
# @mention prefix
...
class MatrixChannelConfig:
    ...
    def __init__(self, raw: dict[str, Any]) -> None:
        ...
        self.device_name: str = raw.get("device_name", "xclaw-worker")
...
class MatrixChannel(BaseChannel):
    """xClaw channel that connects to a Matrix homeserver via matrix-nio."""
    ...
    def _e2ee_store_path(self) -> Path:
        ...
        return Path.home() / ".xclaw" / "matrix_crypto_store"
    ...
    async def _on_room_event(
        ...
        # Skip history prepend for slash commands — xClaw's command parser
    ...
    def build_agent_request_from_native(self, native_payload: Any) -> Any:
        ...
        # in the same room share one session (xClaw keys session state on
