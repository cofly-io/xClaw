...
_REQUIREMENTS_METADATA_NAMESPACES = (
    "openclaw",
    "xclaw",
    "qwenpaw",
    "clawdbot",
...
def _normalize_skill_manifest_entry(entry: Any) -> dict[str, Any]:
    """Return dict entry, fallback to empty dict for malformed legacy data."""
...
def list_builtin_import_candidates() -> list[dict[str, Any]]:
    ...
    for skill_name, source_signature in sorted(builtin_sigs.items()):
        ...
        current = _normalize_skill_manifest_entry(pool_skills.get(skill_name))
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.xclaw.emoji (or legacy metadata.qwenpaw.emoji)."""
    if not isinstance(metadata, dict):
        return ""
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="xclaw_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"xclaw_skill_stage_{skill_name}_"),
    )
