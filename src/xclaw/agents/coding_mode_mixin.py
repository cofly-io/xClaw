# -*- coding: utf-8 -*-
"""Coding Mode mixin for xClawAgent.
...
2. **Prefer targeted edits** — use `edit_file` over full-file rewrites whenever possible.
...
Keep reasoning concise.  Prefer small, verifiable steps over large monolithic changes.
...
The internal xClaw workspace (configs, sessions, memory) is at:
...
class CodingModeMixin:
    ...
    At runtime this class is mixed into ``xClawAgent`` and combined
    ...
    # ------------------------------------------------------------------
    # Tool registration hook (called from xClawAgent._create_toolkit)
    # ------------------------------------------------------------------
