# -*- coding: utf-8 -*-
"""xClaw Agent - Main agent implementation.

This module provides the main xClawAgent class built on ReActAgent,
...
class xClawAgent(CodingModeMixin, ToolGuardMixin, ReActAgent):
    """xClaw Agent with integrated tools, skills, and memory management.
    ...
    MRO note
    ~~~~~~~~
    MRO: xClawAgent → CodingModeMixin → ToolGuardMixin → ReActAgent.
    ...
    def _rebuild_mcp_client(client: Any) -> Any | None:
        ...
        rebuild_info = getattr(client, "_xclaw_rebuild_info", None)
        ...
        try:
            if transport == "stdio":
                ...
                setattr(rebuilt_client, "_xclaw_rebuild_info", rebuild_info)
                return rebuilt_client
    ...
    def _strip_tool_use_from_msg(msg: Msg) -> Msg:
        ...
        if isinstance(msg.content, str):
            msg.content += xClawAgent._ROUND_END_NOTICE
            return msg
        ...
        filtered.append(
            {"type": "text", "text": xClawAgent._ROUND_END_NOTICE},
        )
        msg.content = filtered
        return msg
    ...
    async def reply(
        ...
        logger.info("xClawAgent.reply: max_iters=%s", self.max_iters)
