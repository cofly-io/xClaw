...
- **Stay flexible**: Most questions can be solved by reading docs, source, and configuration. User data (`config.json`, `workspaces/`, etc.) follows the effective **`WORKING_DIR`** (see `src/xclaw/constant.py`): if **`~/.copaw`** still exists on the machine, the process prefers it; otherwise it is typically **`~/.xclaw`**, or a path from **`QWENPAW_WORKING_DIR`** (with **`COPAW_*`** legacy names as fallback). Do **not** assume everything is under `~/.xclaw`; if reads fail, cross-check environment variables and actual paths.
