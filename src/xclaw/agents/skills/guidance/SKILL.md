---
name: guidance
description: "回答用户关于 xClaw 安装与配置的问题：优先定位并阅读本地文档，再提炼答案；若本地信息不足，兜底访问官网文档。"
metadata:
...
当用户询问 **xClaw 的安装、初始化、环境配置、依赖要求、常见配置项** 时，使用本 skill。
...
```bash
DOCS_DIR=$(python3 -c "from xclaw.constant import DOCS_DIR; print(DOCS_DIR or '')" 2>/dev/null)
```
...
```bash
# 获取memory中的文档目录
DOC_DIR=$(find ~/.xclaw/memory/ -type d -name "docs")
```
...
```bash
# 获取二进制绝对路径
COP_PATH=$(which xclaw 2>/dev/null || whereis xclaw | awk '{print $2}')

# 逻辑推导：如果路径包含 .xclaw/bin/xclaw，则根目录在其上三层
# 例如：/path/to/xClaw/.xclaw/bin/xclaw -> /path/to/xClaw
if [[ "$COP_PATH" == *".xclaw/bin/xclaw" ]]; then
    QWENPAW_ROOT=$(echo "$COP_PATH" | sed 's/\/\.xclaw\/bin\/xclaw//')
else
    # 兜底：尝试获取所在目录的父目录
    QWENPAW_ROOT=$(dirname $(dirname "$COP_PATH") 2>/dev/null || echo ".")
fi

echo "Detected xClaw Root: $QWENPAW_ROOT"
```
...
如果还是找不到文档，搜索 xclaw 安装路径下的可用文档内容：
