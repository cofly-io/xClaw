---
name: cron
description: Use this skill only for scheduled or recurring tasks. Manage cron jobs with xclaw cron list/create/get/state/pause/resume/delete/run. Always pass --agent-id explicitly. | 仅在需要未来定时执行或周期执行时使用本 skill。用 xclaw cron list/create/get/state/pause/resume/delete/run 管理任务；必须显式传 --agent-id
metadata:
...
```bash
# 列出任务
xclaw cron list --agent-id <agent_id>

# 查看任务详情
xclaw cron get <job_id> --agent-id <agent_id>

# 查看任务状态
xclaw cron state <job_id> --agent-id <agent_id>

# 创建任务
xclaw cron create --agent-id <agent_id> ...

# 删除任务
xclaw cron delete <job_id> --agent-id <agent_id>

# 暂停 / 恢复任务
xclaw cron pause <job_id> --agent-id <agent_id>
xclaw cron resume <job_id> --agent-id <agent_id>

# 立即执行一次已有任务
xclaw cron run <job_id> --agent-id <agent_id>
```
...
```bash
xclaw cron create --agent-id <agent_id> -f job_spec.json
```
...
- 缺少参数时，先问用户再创建
- 修改/暂停/删除前，先 `xclaw cron list --agent-id <agent_id>`
- 排查问题时，用 `xclaw cron state <job_id> --agent-id <agent_id>`
