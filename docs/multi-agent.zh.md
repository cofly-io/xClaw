...
xClaw 支持**多智能体**，允许您在同一个 xClaw 实例中运行多个独立的 AI 智能体。
...
简单来说，**多智能体**就是让您可以在一个 xClaw 中运行多个"分身"，每个分身：
...
启动 xClaw 后，在控制台**左上角**可以看到**智能体切换器**：
...
**注意**：删除后工作区目录会保留（防止误删数据），如需彻底清理，请手动删除 `~/.xclaw/workspaces/{agent_id}` 目录。
...
如果您之前使用 xClaw **v0.0.x**，升级到 **v0.1.0** 时会**自动迁移**：
