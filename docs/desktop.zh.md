...
本文档说明如何在 Windows 和 macOS 系统上安装和使用 xClaw Desktop 桌面应用。
...
1. **下载安装包**
   从 [Release 页面][releases]下载 `xClaw-Setup-<version>.exe` 文件

2. **运行安装程序**
   双击 `.exe` 文件，按照安装向导提示完成安装
   - 默认安装位置：`C:\Users\<你的用户名>\AppData\Local\xClaw`
...
#### **xClaw Desktop** (推荐日常使用)

- **特点**: 静默启动，无终端窗口，界面简洁
- **适用场景**: 正常使用，不需要查看技术日志
- **启动方式**: 双击桌面或开始菜单的 "xClaw Desktop" 图标
- **技术说明**: 使用 VBScript 启动器，后台运行 Python 进程

#### **xClaw Desktop (Debug)** (调试模式)
...
- **适用场景**:
  ...
  - 开发测试
  - 报告 Bug 时需要提供日志
- **启动方式**: 双击开始菜单的 "xClaw Desktop (Debug)" 图标
- **日志内容**:
  - 应用启动信息
  - Python 错误堆栈
  - API 调用日志
...
**Q: 应用启动后没有反应？**
A: 使用 "xClaw Desktop (Debug)" 模式启动，查看终端输出的错误信息

**Q: 如何卸载？**
A: 在 Windows 设置 → 应用 → 已安装的应用 → 找到 "xClaw Desktop" → 卸载
...
1. **下载压缩包**
   从 [Release 页面][releases]下载 `xClaw-<version>-macOS.zip` 文件

2. **解压缩**
   双击 `.zip` 文件自动解压，得到 `xClaw.app` 应用

3. **移动到应用程序文件夹 (可选)**
   将 `xClaw.app` 拖到 `/Applications` 文件夹

### 首次启动：解除系统安全限制

#### 为什么需要手动信任？

xClaw 应用**未经过 Apple 开发者签名和公证（Notarization）**，macOS Gatekeeper 会默认阻止运行。
...
```bash
# 切换到应用目录
cd /Applications  # 或您的 xClaw.app 所在目录
...
**Q: 如何卸载？**
A: 将 `xClaw.app` 拖到废纸篓，然后删除 `~/.xclaw` 配置文件夹
...
- **GitHub Issues**: [提交问题](https://github.com/agentscope-ai/xClaw/issues)
- **打包文档**: `scripts/pack/README.md` - 技术细节和本地构建指南
- **日志位置**:
  - Windows: Debug 模式终端查看，或 `%USERPROFILE%\.xclaw\` 目录
