...
xClaw 通过一组 Markdown 文件定义智能体的"人设"，这些文件会被加载到系统提示词（System Prompt）中，决定智能体的行为风格、工作方式和个性特征。你可以通过编辑这些文件，让智能体成为符合你需求的专属助手——无论是严谨的工作助理、温暖的生活伙伴，还是技术专家。

---

## 人设文件

xClaw 的人设由 Markdown 文件定义，默认位于智能体工作区目录下。工作区目录的位置取决于 `QWENPAW_WORKING_DIR` 环境变量（默认为 `~/.xclaw`），完整路径为：
...
首次运行 `xclaw init` 时会自动创建 BOOTSTRAP.md，它引导用户和智能体进行初次"对话"，共同定义身份、偏好和风格。完成引导后，智能体会将设定写入 PROFILE.md 和 SOUL.md，然后删除 BOOTSTRAP.md。
...
你也可以直接修改智能体配置文件（`~/.xclaw/workspaces/{agent_id}/agent.json`）中的 `system_prompt_files` 字段来管理人设文件的加载：
...
运行 `xclaw init` 时，系统会根据你选择的语言（`zh` / `en` / `ru`）自动创建模板文件：
...
如果使用 `xclaw init --defaults`，则默认语言为 `zh`（中文）。
...
xClaw 在首次运行 `xclaw init` 时会自动创建一个名为 **"QA Agent"** 的内置智能体（ID：`xClaw_QA_Agent_0.2`）。

### QA 智能体的特点

这是一个**专门用于回答 xClaw 相关问题**的智能体：

- **专属人设**：使用专门为问答优化的人设文件（与普通智能体不同）
- **预装技能**：自动启用 `guidance` 和 `QA_source_index` 技能，可以查询 xClaw 官方文档和源码
- **工具配置**：默认只启用核心工具（execute_shell_command、read_file、write_file、edit_file、view_image），其他内置工具默认禁用
- **自动维护**：每次运行 `xclaw init` 时会自动确保该智能体存在

### 如何使用？

您可以在控制台右上角的智能体切换器中选择 "QA Agent"，然后向它提问关于 xClaw 的任何问题。
...
- **可以删除**：在"设置 → 智能体管理"页面删除（删除后不影响其他智能体，下次 `xclaw init` 会重新创建）
- **工作区位置**：`$QWENPAW_WORKING_DIR/workspaces/xClaw_QA_Agent_0.2/`（默认为 `~/.xclaw/workspaces/xClaw_QA_Agent_0.2/`）
