...
const INDUSTRIAL_CHAT_PREVIEW: { role: "user" | "assistant"; text: string }[] = [
    ...
    { role: "assistant", text: "早班较上周高约 12%，可查 supOS 设备曲线与冷却水路。" },
    { role: "user", text: "报警 A-2048 要停线吗？" },
    { role: "assistant", text: "多为 OPC 短时中断，先看网关与 PLC，短时未恢复再处理。" },
];
                ...
                <div className={styles.leftPanel}>
                    <div className={styles.chatPreview}>
                        <div className={styles.chatPreviewHeader}>
                            <MessageSquare className={styles.chatPreviewHeaderIcon} size={18} strokeWidth={2} />
                            <span className={styles.chatPreviewTitle}>工业场景 · 对话记录</span>
                            {/* <span className={styles.chatPreviewBadge}>
                                <Building2 size={12} strokeWidth={2} aria-hidden />
                                示例
                            </span> */}
                        </div>
                        <div className={styles.chatPreviewList} aria-label="工业场景对话示例">
                            {INDUSTRIAL_CHAT_PREVIEW.map((m, i) =>
                                m.role === "user" ? (
                                    <div key={i} className={`${styles.chatRow} ${styles.chatRowUser}`}>
                                    ...
                                    <div key={i} className={`${styles.chatRow} ${styles.chatRowAssistant}`}>
                                        <div className={styles.chatMeta}>
                    ...
                    <div className={styles.welcomeSection}>
                        ...
                        <div className={styles.reminderMessage}>
                            <span className={styles.reminderIconWrap} aria-hidden>
                                <Coffee className={styles.reminderLucideIcon} size={18} strokeWidth={2} />
                            </span>
                            <span className={styles.reminderText}>{greeting.sub}</span>
                        </div>
                    </div>

                    <Form id="login-card-form" form={loginForm} onFinish={handleLogin} className={styles.loginForm}>
                        ...
                        <Button htmlType="submit" loading={loading} className={styles.loginAction} block>
                            <span>登录 · 进入工作台</span>
                            <ArrowRightOutlined />
                        </Button>
                    </Form>
