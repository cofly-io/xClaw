...
export const PYPI_URL = "https://pypi.org/pypi/xclaw/json";

export const GITHUB_URL = "https://github.com/agentscope-ai/xClaw" as const;
...
export const KEY_TO_LABEL: Record<string, string> = {
  ...
  agents: "nav.agents",
  backups: "nav.backups",
  "plugin-manager": "nav.pluginManager",
};
...
export const getDocsUrl = (lang: string): string =>
  `https://xclaw.agentscope.io/docs/intro?lang=${getWebsiteLang(lang)}`;

export const getFaqUrl = (lang: string): string =>
  `https://xclaw.agentscope.io/docs/faq?lang=${getWebsiteLang(lang)}`;

export const getReleaseNotesUrl = (lang: string): string =>
  `https://xclaw.agentscope.io/release-notes?lang=${getWebsiteLang(lang)}`;
...
docker run -p 127.0.0.1:8088:8088 -v xclaw-data:/app/working agentscope/xclaw:latest
\`\`\`

升级后重启服务 xclaw app。`,

  ru: `### Как обновить xClaw

Чтобы обновить xClaw, выберите способ в зависимости от типа установки:
...
После обновления перезапустите сервис с помощью \`xclaw app\`.`,

  en: `### How to update xClaw
