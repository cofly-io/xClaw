...
def mock_agent_workspace(tmp_path, monkeypatch):
    ...
    import json
    from xclaw.config.utils import get_config_path
    from xclaw.config.config import Config, AgentsConfig, AgentProfileRef

    # Setup workspace directory
    workspace_dir = tmp_path / "workspaces" / "test_agent"
    workspace_dir.mkdir(parents=True, exist_ok=True)

    monkeypatch.setattr("xclaw.config.utils.WORKING_DIR", tmp_path)
    monkeypatch.setattr("xclaw.config.config.WORKING_DIR", tmp_path)
