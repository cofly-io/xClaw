...
from xclaw.cli.channels_cmd import channels_group
from xclaw.cli.cron_cmd import cron_group
from xclaw.cli.daemon_cmd import daemon_group
from xclaw.cli.chats_cmd import chats_group
from xclaw.cli.skills_cmd import skills_group
from xclaw.config.config import AgentProfileConfig
...
def test_channels_list_default_agent(
    ...
    """Test xclaw channels list uses default agent by default."""
    ...
    with patch("xclaw.cli.channels_cmd.load_agent_config") as mock_load:
...
def test_channels_list_custom_agent(
    ...
    """Test xclaw channels list with custom agent_id."""
...
def test_cron_list_with_agent_id():
    """Test xclaw cron list with --agent-id."""
    runner = CliRunner()

    with patch("xclaw.cli.cron_cmd.client") as mock_client:
...
def test_daemon_status_default_agent():
    """Test xclaw daemon status defaults to 'default' agent."""
    runner = CliRunner()

    with patch("xclaw.cli.daemon_cmd.run_daemon_status") as mock_status:
        with patch("xclaw.cli.daemon_cmd._get_agent_workspace") as mock_ws:
...
def test_daemon_status_custom_agent():
    """Test xclaw daemon status with custom agent."""
    runner = CliRunner()
...
def test_skills_list_default_agent():
    """Test xclaw skills list defaults to 'default' agent."""
    runner = CliRunner()

    with patch(
        "xclaw.cli.skills_cmd._get_agent_workspace",
    ) as mock_ws:
        with patch("xclaw.cli.skills_cmd.SkillService") as mock_service:
            mock_ws.return_value = "/tmp/default"
...
def test_skills_list_custom_agent():
    """Test xclaw skills list with custom agent."""
    runner = CliRunner()
...
def test_chats_list_with_agent_id():
    """Test xclaw chats list with --agent-id."""
    runner = CliRunner()

    with patch("xclaw.cli.chats_cmd.client") as mock_client:
        mock_response = MagicMock()
