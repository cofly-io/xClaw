# -*- coding: utf-8 -*-
"""Tests for the ``xclaw task`` headless CLI command."""
...
from xclaw.cli.task_cmd import _read_instruction
...
def test_task_rejects_empty_instruction(monkeypatch) -> None:
    monkeypatch.setattr(
        "xclaw.config.config.load_agent_config",
        MagicMock(),
    )
...
def test_output_dir_writes_result_json(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("xclaw.cli.task_cmd._run_task", _fake_run_task)
...
def _build_guarded_agent(request_context: dict):
    from xclaw.agents.tool_guard_mixin import ToolGuardMixin
...
async def test_tool_guard_not_bypassed_without_flag():
    ...
    with patch(
        "xclaw.security.tool_guard.engine.get_guard_engine",
    ) as mock_engine_fn:
        ...
        with patch("xclaw.app.approvals.get_approval_service"):
...
def test_e2e_cli_no_guard_and_skills_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("xclaw.cli.task_cmd._run_task", _spy_run_task)
...
def test_isolated_workspace_creates_overlay(tmp_path):
    ...
    from xclaw.cli.task_cmd import _isolated_skills_workspace
    from xclaw.agents.skills_manager import resolve_effective_skills
