# -*- coding: utf-8 -*-
"""Tests for the ``xclaw agents`` CLI surface."""
...
def test_agents_list_uses_shared_tool_helper(monkeypatch) -> None:
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd.agent_tools.list_agents_data",
        lambda _base_url: {
            "agents": [
                {
                    "id": "bot_a",
...
def test_agents_chat_uses_shared_request_builder(monkeypatch) -> None:
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd.agent_tools.build_agent_chat_request",
    ...
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd.agent_tools.collect_final_agent_chat_response",
        lambda *_args, **_kwargs: {
            "output": [
                {
                    "content": [
...
def test_agents_create_uses_explicit_agent_id(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("xclaw.cli.agents_cmd.load_config", lambda: config)
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd.save_config",
        lambda updated_config: saved.setdefault("config", updated_config),
    )
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd.save_agent_config",
    ...
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd._initialize_new_agent_workspace",
...
def test_agents_create_sets_active_model_when_requested(
    ...
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd._build_active_model_config",
...
def test_agents_delete_calls_local_api(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd.agent_tools.create_agent_api_client",
...
def test_agents_delete_remove_workspace_deletes_directory(
    ...
    monkeypatch.setattr("xclaw.cli.agents_cmd.WORKING_DIR", tmp_path)
...
def test_agents_delete_rejects_workspace_outside_working_dir(
    ...
    allowed_root.mkdir()
    monkeypatch.setattr(
        "xclaw.cli.agents_cmd.WORKING_DIR",
        allowed_root,
    )
