...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repo-black.svg?logo=github)](https://github.com/agentscope-ai/xClaw)
[![PyPI](https://img.shields.io/pypi/v/xclaw?color=3775A9&label=PyPI&logo=pypi)](https://pypi.org/project/xclaw/)
[![Documentation](https://img.shields.io/badge/Docs-Website-green.svg?logo=readthedocs&label=Docs)](https://xclaw.agentscope.io/)
...
[![Last Commit](https://img.shields.io/github/last-commit/agentscope-ai/xClaw)](https://github.com/agentscope-ai/xClaw)
[![License](https://img.shields.io/badge/license-Apache%202.0-red.svg?logo=apache&label=License)](LICENSE)
[![Code Style](https://img.shields.io/badge/code%20style-black-black.svg?logo=python&label=CodeStyle)](https://github.com/psf/black)
[![GitHub Stars](https://img.shields.io/github/stars/agentscope-ai/xClaw?style=flat&logo=github&color=yellow&label=Stars)](https://github.com/agentscope-ai/xClaw/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/agentscope-ai/xClaw?style=flat&logo=github&color=purple&label=Forks)](https://github.com/agentscope-ai/xClaw/network)
...
[[Documentation](https://xclaw.agentscope.io/)] [[中文](README_zh.md)] [[日本語](README_ja.md)] [[Русский](README_ru.md)]

<p align="center">
  <img src="https://gw.alicdn.com/imgextra/i1/O1CN01sens5C1TuwioeGexL_!!6000000002443-55-tps-771-132.svg" alt="xClaw Logo" width="120">
</p>
...
>
> **Skills extension** — Built-in scheduling, PDF/Office processing, news digest, and more; custom skills auto-loaded, no lock-in. Skills determine what xClaw can do.
>
...
[2026-04-12] **xClaw is Officially Rebranding to xClaw**: This rebranding marks an important step forward into our next phase of open-source development.
...
From today on, xClaw will stay and grow with you!
...
> - **💬 I want to use in DingTalk/Feishu/WeChat**: Complete Quick Start → [Configure model](#api-key) → [Channel setup](https://xclaw.agentscope.io/docs/channels)
> - **🐍 I don't want to install Python**: [Desktop app](#option-6-desktop-application-beta) or [Script install](#option-2-script-install) or [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/xClaw)
...
Then open the Console in your browser at **http://127.0.0.1:8088/** to configure your model. To chat in DingTalk, Feishu, WeChat, etc., see the [Channel setup](https://xclaw.agentscope.io/docs/channels) documentation.
...
No Python setup required, one command installs everything. The script will automatically download uv (Python package manager), create a virtual environment, and install xClaw with all dependencies (including Node.js and frontend assets). Note: May not work in restricted network environments or corporate firewalls.

**macOS / Linux:**

```bash
curl -fsSL https://xclaw.agentscope.io/install.sh | bash
```

To install with Ollama support:

```bash
curl -fsSL https://xclaw.agentscope.io/install.sh | bash -s -- --extras ollama
```

To install with multiple extras (e.g., Ollama + local):

```bash
curl -fsSL https://xclaw.agentscope.io/install.sh | bash -s -- --extras ollama,local
```

**Windows (CMD):**

```CMD
curl -fsSL https://xclaw.agentscope.io/install.bat -o install.bat && install.bat
```

**Windows (PowerShell):**

```powershell
irm https://xclaw.agentscope.io/install.ps1 | iex
```
...
>      - Check if `uv` is available: Enter `uv --version` in CMD. If a version number appears, **only configure the xClaw path**. If you receive the prompt `'uv' is not recognized as an internal or external command, operable program or batch file,` configure both paths.
...
>      - xClaw path: Typically located at `%USERPROFILE%\.xclaw\bin`.
...
>   - **Configure `uv` environment variables**: Add the `uv` directory and `%USERPROFILE%\.xclaw\bin` to your system's `Path` variable.
>   - **Re-run the installation**: Open a new terminal and execute the installation script again to complete the `xClaw` installation.
>   - **Configure the `xClaw` environment variable**: Add `%USERPROFILE%\.xclaw\bin` to your system's `Path` variable.

Once installed, open a new terminal and run:

```bash
xclaw init --defaults   # or: xclaw init (interactive)
xclaw app
```
...
# Uninstall
xclaw uninstall          # keeps config and data
xclaw uninstall --purge  # removes everything
```
...
Images are on **Docker Hub** (`agentscope/xclaw`). Image tags: `latest` (stable); `pre` (PyPI pre-release).
...
>
> Inside a Docker container, `localhost` refers to the container itself, not your host machine. If you run Ollama (or other model services) on the host and want xClaw in Docker to reach them, use one of these approaches:
>
...
If you're not comfortable with command-line tools, you can download and use xClaw's desktop application without manually configuring Python environments or running commands.

#### Download

Download the desktop app from [GitHub Releases](https://github.com/agentscope-ai/xClaw/releases):
- **Windows**: `xClaw-Setup-<version>.exe`
- **macOS**: `xClaw-<version>-macOS.zip` (Apple Silicon recommended)
...
When you download the xClaw macOS app from Releases, macOS may show: *"Apple cannot verify that 'xClaw' contains no malicious software"*. This happens because the app is not notarized. You can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the xClaw app → **Open** → in the dialog click **Open** again. This tells Gatekeeper you trust the app; after that you can double-click to launch as usual.

- **Allow in System Settings**
  If it is still blocked, go to **System Settings → Privacy & Security**, scroll to the message like *"xClaw was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal run:
  `xattr -cr /Applications/xClaw.app`
...
For detailed usage instructions, troubleshooting, and common issues, see the [Desktop Application Guide](https://xclaw.agentscope.io/docs/desktop).

---

## API Key

If you use a **cloud LLM API** (e.g., Qianwen, Gemini, OpenAI), you must configure an API key before chatting. xClaw will not work until a valid key is set. See the [official docs](https://xclaw.agentscope.io/docs/models) for details.

**How to configure:**

1. **Console (recommended)** — After running `xclaw app`, open **http://127.0.0.1:8088/** → **Settings** → **Models**. Choose a provider, enter the **API Key**, and enable that provider and model.
2. **`xclaw init`** — When you run `xclaw init`, it will guide you through configuring the LLM provider and API key. Follow the prompts to choose a provider and enter your key.
...
Tools that need extra keys (e.g. `TAVILY_API_KEY` for web search) can be set in Console **Settings → Environment variables**, see [Config](https://xclaw.agentscope.io/docs/config) for details.

> **Using local models only?** If you use [Local Models](#local-models) (llama.cpp / Ollama / LM Studio), you do **not** need any API key.

## Local Models

xClaw can run LLMs entirely on your machine — no API keys or cloud services required. See the [official docs](https://xclaw.agentscope.io/docs/models) for details.
...
| Topic                                                                 | Description                                      |
| --------------------------------------------------------------------- | ------------------------------------------------ |
| [Introduction](https://xclaw.agentscope.io/docs/intro)                | What xClaw is and how to use it                  |
| [Quick start](https://xclaw.agentscope.io/docs/quickstart)            | Install and run (local or ModelScope Studio)    |
| [Console](https://xclaw.agentscope.io/docs/console)                   | Web UI: chat and agent configuration            |
| [Models](https://xclaw.agentscope.io/docs/models)                     | Configure cloud, local, and custom providers    |
| [Channels](https://xclaw.agentscope.io/docs/channels)                  | DingTalk, Feishu, QQ, Discord, iMessage, and more |
...
| [Multi-Agent](https://xclaw.agentscope.io/docs/multi-agent)           | Create multiple agents and enable collaboration  |
| [Config & working dir](https://xclaw.agentscope.io/docs/config) | Working directory and config file                |
| [CLI](https://xclaw.agentscope.io/docs/cli)                            | Init, cron jobs, skills, clean                   |
| [FAQ](https://xclaw.agentscope.io/docs/faq)                           | Common questions and troubleshooting             |
...
xClaw includes multi-layer security mechanisms to protect your data and system:
...
- **Web Authentication** — Optional login protection for the Console. Disabled by default; set `QWENPAW_AUTH_ENABLED=true` to enable. See [Web Authentication](https://xclaw.agentscope.io/docs/security#Web-Authentication) for details.

See [Security documentation](https://xclaw.agentscope.io/docs/security) for details.

---

## FAQ

For common questions, troubleshooting tips, and known issues, please visit the **[FAQ page](https://xclaw.agentscope.io/docs/faq)**.

---

## Staying ahead

<a href="https://github.com/agentscope-ai/xClaw">
  <img src="https://img.alicdn.com/imgextra/i1/O1CN01V8HYv61By0HYcIDaq_!!6000000000013-1-tps-1698-954.gif" width="600" alt="Star xClaw" />
</a>

Star xClaw on GitHub and be instantly notified of new releases.
...
```bash
git clone https://github.com/agentscope-ai/xClaw.git
cd xClaw
...
mkdir -p src/xclaw/console
cp -R console/dist/. src/xclaw/console/
...
- **Then**: Run `xclaw init --defaults`, then `xclaw app`.

> **Note for updates:** When updating to a new major version after `git pull`, please also rebuild the frontend, reinstall the package (`pip install -e .`), restart `xclaw app`, and clear your browser cache with `Ctrl+Shift+R` (or `Cmd+Shift+R` on macOS).

---

## Contributing

xClaw evolves through open collaboration, and we welcome all forms of contribution! Check the [Roadmap](#roadmap) above (especially items marked **Seeking Contributors**) to find areas that interest you, and read [CONTRIBUTING](https://github.com/agentscope-ai/xClaw/blob/main/CONTRIBUTING.md) to get started. We particularly welcome:
...
Join [GitHub Discussions](https://github.com/agentscope-ai/xClaw/discussions) to discuss ideas or pick up tasks.

---

## Why xClaw?

xClaw stands for Qwen Personal Agent Workstation, and also embodies the wisdom of Qwen and the warmth of a Paw.
...
xClaw collects **anonymous** usage data during `xclaw init` to help us understand our user base and prioritize improvements. Data is sent **once per version** — when you upgrade xClaw, telemetry is re-collected so we can track version adoption.

**What we collect:**

- xClaw version (e.g., 0.0.7)
...
When running `xclaw init` interactively, you will be asked whether to opt in. If you choose `--defaults`, telemetry is accepted automatically. The prompt appears once per version and never affects xClaw's functionality.

---

## License

xClaw is released under the [Apache License 2.0](LICENSE).
...
<a href="https://github.com/agentscope-ai/xClaw/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=agentscope-ai/xClaw" alt="Contributors" />
</a>
